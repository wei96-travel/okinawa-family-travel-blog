...
- 2026-08-14 **AdSense 審查未通過（重要狀態更正，Claude Code 記錄）**：`adsense-noreply@google.com` 於 2026-08-13 19:09 寄出「您必須修正部分問題，網站才能開始使用 AdSense」，內容為「負責團隊已審查完畢，但很抱歉，您的網站目前無法放送廣告。您的網站有些問題，必須先修正才能開始放送廣告」，發佈商編號 `pub-8772944565154374`。**本檔先前多筆條目記載的「site under review／正在接受審查」已經過期，不可再沿用。** 信件本身沒有寫出具體原因，只指向帳戶內的「網站」頁面，因此確切退件理由必須登入 AdSense 帳戶查看。Claude Code 無法取得該資訊：Claude in Chrome 擴充功能未連線，內建瀏覽器則被政策封鎖 Google 網域；未嘗試登入、未輸入任何憑證、未點選「申請審查」。**可從外部驗證的項目全部正常**（2026-08-14 實測）：`/privacy`、`/about`、`/contact`、`/blog`、首頁皆回 200；`ads.txt` 內容為正確的 `google.com, pub-8772944565154374, DIRECT, f08c47fec0942fa0`；`robots.txt` 允許檢索並列出 sitemap；首頁 HTML 含發佈商編號與 `adsbygoogle` 指令碼，且無 `noindex`；隱私權頁涵蓋 Google 廣告與 Cookie 說明。因此退件原因應屬帳戶頁面所列的政策或內容判定，需人工查看後才能決定下一步。在取得確切原因前，不要建立廣告版位、不要重新申請審查、也不要在任何紀錄中宣稱網站審查中或即將放送廣告。

- 2026-08-14 **Search Console 重新導向錯誤（Claude Code 已初步排除站台設定問題）**：`sc-noreply@google.com` 於 2026-08-09 通知「重新導向錯誤」為網頁無法編入索引的新原因。2026-08-14 實測結果：`next.config.mjs` 目前設定的 7 條 redirect 全部健康，`http://` 與 `www.` 皆一次跳轉到 `https://okinawafamilynotes.com` 並回 200，`/car-seat`、舊的 `/blog/okinawa-driving-rest-stops-family` 與 `/previews/*` 也都是一次跳轉回 200，沒有迴圈或斷鏈；`sitemap.xml` 的 71 個網址中沒有任何會重新導向的網址。因此問題不在站台目前的 redirect 設定或 sitemap，必須從 Search Console 索引報表的「重新導向錯誤」清單取得 Google 實際點名的網址才能定位。Claude Code 同樣無法存取 Search Console（原因同上）。未提交任何索引要求。
