...
- 題目依據：Keyword Planner 既有紀錄顯示 `沖繩親子公園怎麼選` 為 100～1,000／月、近三個月 +900%；本輪先查官方資料與站內搜尋意圖重疊，完成研究與可審閱草稿，不直接發布、不新增分潤連結，也不干擾 2026-10-05 前的兒童王國單一 CTA 實驗。
- 研究筆記與完整草稿已完成。四園分工為：DINO＝一小時恐龍短景點、Neo Park＝活體動物／小火車／互動、兒童王國＝半天至全天且有室內調整點、名護鳳梨園＝乘車／餐飲／採買的輕鬆停靠。五條站內連結均已確認目標存在，草稿沒有合作連結或發布占位符。
- 事實查核只採園區官網；名護鳳梨園首頁仍有舊價區塊，本文改以官方設施頁的現行資料為準。Neo Park 9/28 後入園與套票價仍依既定規則留待 9/28 再查，今天不提前推定。
- 2026-09-27 已向 Claude Code 傳送不含本機檔案與個資的公開來源摘要，要求唯讀第二審；程序連續 90 秒沒有輸出後中止，因此本輪沒有 CC 結論，不可記成通過。正式發布前仍需完成第二審或如實記錄阻擋。
- 下一步：9/28 先核對 Neo Park 官網與官方預約頁，再修正比較稿與 Neo Park 專文；完成事實、語感與手機結構檢查後，把比較稿完整版本交 wei96 明確確認。確認前不移入 `content/blog/`、不做封面、不發布。

> **開工前先看這一區。** 這是摘要，方便快速進入狀況；證據與完整脈絡在下方各則日期紀錄裡。**兩邊衝突時以日期紀錄為準。** 做完事情要回來更新這一區。

## 檢查點日曆

| 日期 | 讀什麼 | 對照基準 | 判讀規則 |
| :-- | :-- | :-- | :-- |
| **2026-09-28 起** | 園方官網料金頁 `https://www.neopark.co.jp/contact/` 的入園料 | 季節料金（4/25～9/27）大人 1,800／小人 1,000，9/27 結束 | 換成新票價後才更新 Neo Park 草稿並走發布流程。料金頁的「入園料(割引料金) 通常 大人 1,600 小人 800」**不可**直接當成季節後票價，該區塊標題是「割引料金」且並列北部居民價，需另外確認 |
| **2026-10-05** | GA4 `affiliate_click` ＋ Klook 後台 Ad ID `1443707`（兒童王國門票） | 2026-08-24～09-20：全站 853 活躍使用者換到 **1 人 2 次** `affiliate_click`、0 訂單 | 點擊起來 → 出口不足，把同樣做法複製到嬰兒用品採買頁與國際通頁；沒起來 → 問題是查詢意圖，長尾資訊文就是天花板，要改搶購買意圖關鍵字 |
| **2026-10-07** | Kit 表單「沖繩伴手禮回程打包卡」（uid `94acbb7f26`）的瀏覽／訂閱／轉換率 | 舊表單「沖繩租車證件檢查表」90 天 **1,400 瀏覽、0 訂閱**；GA4 28 天 273 人看到表單、3 人點進輸入框 | 轉換率有起來 → 誘餌錯配是主因，依序處理其他高流量頁；沒起來 → 問題不在誘餌，要回頭查表單本身或流量意圖 |
| **2026-10-21** | 租車群的「查詢 × 頁面」明細，**不要看混合後的平均排序** | 2026-08-24～09-20：樞紐頁本身 1 曝光；含「租車」查詢 33 曝光、0 點擊、平均排序 19.1，且**全部落在子頁**；主詞「沖繩租車」全站 0 曝光 | 樞紐頁開始因自己的詞（哪一家／一天多少錢）拿到曝光 → 身分修正有效；子頁排名變穩也算有效 |
| **2026-10-22** | Kit 成效（Codex automation 已排程提醒） | 啟用時訂閱 0、分潤 0；方案 Creator Monthly **US$39／月** | 排除站方測試後決定續用，或在 **10-25 扣款前**降級 |
...
## 站台現況（括號內是量測日，不要當成即時數字）

- 內容：58 篇文章，`content:audit` 0 重大問題、25 則既有口吻提醒（2026-09-23）
- 搜尋：28 天 713 點擊／1.82 萬曝光／CTR 3.9%／平均排序 6.5；索引 79、未索引 9、404 歸零（2026-09-21）
- 變現：三個聯盟平台 30 天 **0 筆訂單、0 元**（2026-09-21）
- 名單：Kit 訂閱 **0**（2026-09-25）
...
- 重要風險：Klook FunPASS 行銷文字稱包含恐龍園，但同頁現行適用項目清單沒有 DINO。未取得現行明細證據前，不得宣稱套票可用。
- 已完成自製封面 `public/images/articles/okinawa-dino-park-family-guide/okinawa-dino-park-cover.webp`（1600×900），frontmatter 清楚標註為 AI 情境圖、非園區實景。
- 已建立並實測本文唯一 Klook 追蹤連結：`aid=128859`、`aff_adid=1456847`，正確轉到活動 `32874`。不放 FunPASS 連結。
- 官方已確認：路線約 30～40 分鐘、80 隻以上恐龍、9:00～18:00（最晚 17:30）、成人 1,000／4～15 歲 600 日圓；推車、雨天與森林步道無障礙範圍尚無官方明文。
- 完整草稿已建立，定位為「只玩 40 分鐘值不值得」，不是一般景點照片遊記；六條站內連結目標均存在，未放普通 Klook 連結。
- 已取得 CC 去識別化第二審。採納停留時間邊界、單票決策位置與推車／雨天風險；不採納「FunPASS 目前不能用」與用固定年齡判斷適合度，因證據不足。理由已記入研究筆記。
...
- wei96 明確要求把它當成第二個獨立事業，不併入沖繩站。Codex 已用公開資料比較需求、競爭缺口、第一手證據、分潤條件與工具成本，正式方案在 `100_Todo/plans/2026-09-26-second-tool-site-plan.md`。
- 目前建議定位是「台灣非技術型個人創作者的工具設定與排錯實測站」：AI 做流量入口，SEO／網站排錯做長期需求，電子報平台與自動化做高意圖成交內容。不是泛 AI 清單、網賺站或單一 GetResponse 推廣站。
- 尚未做的決策門：Keyword Planner 台灣絕對搜尋量與 CPC、20 個 SERP 人工難度、品牌／網域可用性、主機續約與退款比較。這些未完成前，不買網域與主機。
- Codex 已於 2026-09-26 對 Claude Code 發出一次完整去識別第二審，另用最小 `CC_READY` 測試；兩次均無任何輸出並已中止。`claude.cmd --version` 與 `claude.cmd doctor` 正常，因此本輪**沒有取得 Claude Code 審查結論**，不可宣稱 CC 同意。Claude Code 下次接手請唯讀審查該方案的定位、首批題群、KPI 與 90 天停損，不要購買、建站或修改沖繩內容。
- 本輪沒有修改公開文章、網站程式、聯盟連結、電子報、社群或部署。

## 2026-09-26 Claude Code 收工交接（無檔案認領，wei96 將與 Codex 討論）

**Active owner：無。** Claude Code 本輪只做唯讀第二審與交接檔維護，**未修改任何 Codex 認領的草稿、研究筆記或計畫檔**。工作區對 `content/` 與 `100_Todo/drafts/` 是乾淨的。

### 本輪推送（只動 `AI_HANDOFF.md`）

- `1e49186` 兩篇 9/25 草稿的第二審結果
- `ec06c23` 檔案最上方新增速查區（檢查點日曆、凍結頁面、站台現況、已知做不到的事）
- `623dd0c` **撤回**第二審中「Codex 把 Neo Park 日期寫錯」的指控——錯的是 Claude Code
- `9b00da9` 記錄 wei96 決定 Neo Park 等 9/28，並把 9/28 加進檢查點日曆

### 交給 wei96 與 Codex 討論的三件事

1. **Neo Park 由誰在 9/28 之後接手改寫。** 檔案仍屬 Codex 認領。已驗證可直接使用的數字、以及「料金頁的 1,600／800 不可當成季節後票價」的但書，都寫在 `623dd0c` 那一則。
2. **住宿草稿與 `how-to-choose-family-hotel` 的角色分工尚未決定。** Claude Code 的建議是舊文保留「選飯店的通則」並加一條內鏈連到新文，新文專做「公寓式還是度假飯店」。未決定之前不要發布，否則重演 2026-09-23 租車樞紐頁兩頁互搶的問題。
3. **本週產出 0 篇這件事要不要接受。** 兩篇都只是延後不是撤回；建議不要為了補篇數另開新題。

### 不是 Claude Code 建立、未提交的檔案

- `100_Todo/plans/2026-09-26-second-tool-site-plan.md`（2026-09-26 00:47，15 KB，第二品牌「台灣創作者工具實測站」90 天驗證方案，狀態寫「提案完成，尚未購買網域」）。**Claude Code 沒有提交、沒有還原、沒有移動它**，目前仍是未追蹤狀態，要不要納入版控請 wei96 決定。

### 下一個接手的人先看哪裡

檔案最上方的速查區。四個檢查點（09-28 Neo Park 票價、10-05 兒童王國分潤、10-07 伴手禮誘餌、10-21 租車群、10-22 Kit 續費）都寫明了要讀什麼數字、對照什麼基準、兩種結果各代表什麼，不需要往下翻舊紀錄考古。

## 2026-09-26 wei96 決定：Neo Park 等 9/28 之後再寫（Claude Code 記錄）

- **wei96 決定不趕在季節料金結束前發布。** Neo Park 草稿留在 `100_Todo/drafts/articles/`，本週不移入 `content/blog/`、不發布。理由：季節料金 4/25～9/27 於明天結束，今天發等於明天就要再改一次票價。

- **9/28 之後要做的事**（接手的人照這個順序）：(1) 回 `https://www.neopark.co.jp/contact/` 讀季節結束後的入園料，並同時開 `https://book.neopark.co.jp/` 對照套票價，兩邊不一致時用單項加總驗算判斷哪一邊是現行的；(2) 把新票價與已驗證的單項價（軽便鉄道大人 1,000／小人 800、ふれあい広場 4 才以上 500）寫進草稿的〈門票怎麼買〉與〈要不要加小火車〉；(3) 一併處理第二審其餘各點（裸 klook 連結改純文字、租車樞紐錨文字改回 2026-09-23 的標準寫法、ふれあい広場提為與小火車並列的第二個決策點）；(4) 再走 wei96 看完整版 → 封面 → 追蹤連結 → 發布驗證的流程。

- **本週產出因此是 0 篇。** 住宿草稿仍卡在新舊文角色分工未定（與 `how-to-choose-family-hotel` 的關係），Neo Park 等票價更新。兩篇都沒有被撤回，只是延後。原「每週 2 篇」的節奏本週不成立，不要為了補篇數另外硬開新題。

- **檔案認領不變**：兩篇草稿仍屬 Codex 本輪認領，Claude Code 至今未修改任何一個草稿檔。若要由 Claude Code 接手改寫，需要 wei96 明講移交。

## 2026-09-26 更正：Neo Park 的 9/27 是對的，是 Claude Code 讀錯來源（Claude Code）

- **撤回下一則第二審裡「草稿日期寫錯」的指控。Codex 寫的 4/25～9/27 正確，草稿不需要因此改。** 錯的是 Claude Code：昨天只讀了預約系統 `book.neopark.co.jp` 頁尾的註記（該處寫 4/25～8/31），沒有回園方官網的料金頁交叉比對就下結論。

- **園方官網料金頁 `https://www.neopark.co.jp/contact/`（2026-09-26 讀取）原文**：「※4/25～9/27までは企画展を含むシーズン料金(気になる木展を含む料金）となります」。入園シーズン料金：大人（13歳以上）1,800 円、小人（4歳～12歳）1,000 円。與草稿一致。

- **兩個園方頁面互相矛盾，判定以料金頁為準的依據是算術**：料金頁的單項為 軽便鉄道 大人 1,000／小人 800、ふれあい広場 4 歳以上一律 500（4 歳未満免費但需家長陪同）。預約系統目前販售的套票為 入園券 1,800、入園＋鉄道 2,800、入園＋ふれあい 2,300、三項全含 3,000（各線上預售折 100）。1,800＋1,000＝2,800、1,800＋500＝2,300 完全吻合，代表預約系統此刻套用的正是 1,800 的季節價，季節期間仍在進行中。故 `book.neopark.co.jp` 的 8/31 註記研判為未更新的舊文字。

- **草稿「若在 9 月 28 日後上線要重新核對」這句是對的而且重要**：季節料金 9/27 結束，**今天是 9/26，只剩明天**。料金頁另列「入園料(割引料金) 通常 大人 1,600 円 小人 800 円」，但該區塊標題是「割引料金」、下方並列北部 12 市町村居民價，**無法確定 1,600／800 是季節結束後的平常價還是另一種折扣**，不要直接當成 9/28 之後的票價寫進文章，要另外向園方確認或等官網更新。

- **可以直接補進草稿的已驗證數字**（兩個園方頁面交叉比對過）：小火車乘車料 大人（中学生以上）1,000 円、小人（4才～小学生）800 円，始発 10:30（日祝 10:00）、最終 16:45（日祝 17:00）；ふれあい広場 10:00–17:00、最終入場 16:45、4 才以上一律 500 円；国際種保存研究センター 免費、10:00–17:15；年中無休（颱風暴風警報時休園）；可自備飲食但展示區禁止；不可帶寵物；前売り券沒有優先入場。

- **第二審其餘各點不受影響**（站上零重疊、FAQ 無撞號、分潤佔位符正確、裸 klook 連結建議改純文字、租車樞紐錨文字要用 2026-09-23 的標準寫法、住宿題需先定位）。

- **教訓**：園方的預約系統與官網料金頁可能不同步，票價類事實要兩邊都讀。單一官方頁面不等於交叉比對。

## 2026-09-26 Claude Code 第二審：Neo Park 票價段需重做，住宿題需先定位（唯讀，未改任何檔案）

- **未修改任何 Codex 認領的檔案。** 本段只記錄審查結果與查證到的數字，供 wei96 與 Codex 決定後續。

- **Neo Park 草稿有一個實質錯誤，發布前必須改。** 草稿第 55 行寫「官方預約頁顯示 4 月 25 日至 9 月 27 日為季節票價」。Claude Code 於 2026-09-26 回 `https://book.neopark.co.jp/` 讀到的原文是「※4/25～**8/31**までは企画展を含むシーズン料金となります」。是 8/31 不是 9/27，代表該期間將近一個月前就結束，草稿「9/28 之後上線要重查」的整段邏輯不成立。

- **同日於官方預約頁讀到的現行票價**（定價 → 線上預售，四種方案並列）：入園券 1,800 → 1,700；入園券＋沖縄軽便鉄道 2,800 → 2,700；入園券＋ふれあい広場 2,300 → 2,200；三項全含 3,000 → 2,900。草稿把 1,800 當成會到期的季節票價，但它看起來就是現行的一般入園票價。**由此可得小火車的實際加價是 1,000 日圓**，而這個數字正是全文主軸「要不要加小火車」的關鍵，草稿目前沒有。

- **官方每個套票都含 ふれあい広場**（水豚、象龜、小型犬，10:00–17:00、最終入場 16:45），對親子讀者的吸引力不亞於小火車，草稿只帶到兩次。建議把它提為與小火車並列的第二個決策點。

- **Neo Park 其餘部分沒有發現問題**：站上零重疊（全站沒有任何文章提過 Neo Park／ネオパーク／名護自然動植物），FAQ 沒有與既有文章標題撞號，分潤佔位符寫法正確。

- **住宿草稿沒有查到事實錯誤，但有定位風險。** 站上已有 7 篇住宿文，最接近的是 `how-to-choose-family-hotel`（沖繩親子飯店怎麼選？不要只看海景）。該頁很薄：五個短章節、洗衣只有兩句、沒有廚房、沒有公寓式與度假飯店的比較、沒有連住天數。新草稿是把同一軸做深。兩頁可以並存，但**必須先講清楚角色**，否則重演 2026-09-23 租車樞紐頁的問題（樞紐與子頁互搶，Google 兩個都不選）。建議：舊文保留「選飯店的通則」並加一條連到新文的內鏈，新文專做「公寓式還是度假飯店」。新草稿的四題 FAQ 已逐一比對，沒有與既有文章標題撞號。

- **兩個小修正**：(1) Neo Park 的〈資料來源〉有一條裸的 `klook.com` 連結，包車文的做法是平台頁只寫文字不做超連結，避免讀者從來源清單點出去產生無歸因點擊，建議比照。(2) Neo Park 連到租車樞紐頁的錨文字是「沖繩親子租車怎麼選」，不是 2026-09-23 統一後的「沖繩租車怎麼選？三家接駁、營業時間與費用比較」；統一完兩天就開始漂移，建議改回標準寫法。

- **撤回一個中途的錯誤判斷**：審查過程中 Claude Code 一度認為兩篇都沒有分潤出口，讀完全文後確認兩篇都有正確的佔位符（核可後才產生追蹤連結），該判斷撤回，不要當成待辦。

- **待 wei96 決定**：Neo Park 票價段由 Codex 續修還是移交 Claude Code；以及住宿題的新舊文角色分工要不要照上述建議走。在 wei96 指示之前，Claude Code 不會動這些檔案。
...
- 2026-09-23 souvenir lead magnet live (Claude Code; claim released, no active owner). Work commit `7cf4661` adds `app/checklist/okinawa-souvenir-packing-card/page.tsx`, a new `okinawa_souvenir_packing_card` entry in `components/newsletter-signup.tsx`, and two slugs in the offer map in `app/blog/[slug]/page.tsx`. 伴手禮打包 and 伴手禮海關 now end with a 回程打包卡 instead of the driving-licence checklist. The card runs from the shop shelf to the customs channel in six stages, plus a box on the four canning conditions and one on the penalties; every fact is taken from those two articles, source-checked 2026-09-09 (customs items) and 2026-07 (packing rules), and nothing new was researched. Kit form 9948948, uid `94acbb7f26`, named 沖繩伴手禮回程打包卡, auto-confirm on, redirect after subscribe and after confirming both pointing at the card, confirmation email rewritten in Traditional Chinese to match the other two. Kit now holds exactly three forms and no duplicate was created. Build passed at 58 articles with the same 25 style reminders. Production verified in a real browser (curl gets a Vercel security checkpoint on the new route): the card returns its own title with `noindex, nofollow` and a self-canonical, is absent from the sitemap, renders all six stages, and at 390px does not scroll sideways with nothing overflowing; 伴手禮打包 and 伴手禮海關 both post to `app.kit.com/forms/9948948`, the new heading is present and the old one gone, while 推車 and 兒童王國 still post to 9801697 and the zoo page still carries aff_adid 1443707. **Baseline to beat:** Kit's report for form 9801697 was 1.4K visitors and 0 subscribers over 90 days, and GA4 28 days showed 273 `newsletter_signup_view` users against 3 `form_start` users. **Do not touch the two souvenir pages before 2026-10-07**, then read the new form's own visitors/subscribers/conversion against that. A note on method: Kit's dialogs kept failing under automation because Chrome's zoom drifts (innerWidth 1920 against a 1568 screenshot frame); computing an element's rect and scaling by frame/innerWidth made every step work first time.

- 2026-09-23 ACTIVE OWNER: Claude Code, claimed before editing. Kit's own form report shows the 租車證件檢查表 form drew 1.4K visitors in 90 days and **zero** subscribers, and GA4 shows 273 people saw the block in 28 days while 3 started typing. The block is seen; the offer is not wanted. The cause is that 55 of 58 articles serve the same car-documents checklist regardless of topic. wei96 chose the souvenir cluster for the first matched magnet: 伴手禮打包 (57 clicks in 28 days) and 伴手禮海關 (46) are the largest pair, and neither is under a running experiment. The 停留時間 cluster was rejected for now because it would touch `okinawa-zoo-museum-family-guide`, which must stay untouched until the 2026-10-05 affiliate reading. Files claimed: a new `app/checklist/okinawa-souvenir-packing-card/page.tsx`, `components/newsletter-signup.tsx`, `app/blog/[slug]/page.tsx` (the slug-to-offer map only) and this handoff. **Codex: please do not touch the newsletter component or the checklist pages while this runs.** Every fact on the card comes from the two existing articles, which were themselves source-checked; nothing new is researched here. A new Kit form is needed, and Kit's settings dialog has already failed repeatedly under automation, so if it fails again the form creation goes to Codex or to wei96 rather than being retried indefinitely. Read the result no earlier than 2026-10-07 in Kit's form report for the new form against the 1.4K/0 baseline of the old one.

- 2026-09-22 zoo/museum affiliate exit live (Claude Code; claim released, no active owner). Work commit `738ce62` adds one Klook ticket link to `content/blog/okinawa-zoo-museum-family-guide.md` and replaces the empty 要先買票嗎 answer with a real one. Ad ID `1443707` (aid 128859, activity 41471, labels `blog_zoo_museum_family` / `zoo_museum_ticket`), created in Chrome after wei96 logged in; five fields checked programmatically before saving, every character of the generated URL read back at zoom, and **not test-clicked**. KKday sells the same ticket at NT$181 with free cancellation a day ahead, but wei96 checked and that affiliate account is not activated, so the article names KKday without a link and says so. Facts read at source on 2026-09-21: okzm.jp/fee gives 1,000 yen for 16 and over, free under 16, stroller hire 500 yen, and cash-only admission at 東第2ゲート alone; the park's 事前購入チケット link points at asoview.com rather than a park system, checked by reading the href. A read-only Codex review returned 修改後可發布 with six points, and four of them were Claude Code's own claims rather than the sources': the saving cannot be demonstrated across two currencies, the pre-purchase set is not the park's own, 掃 QR code 進場 is not what the Klook page says (it says show the voucher and let staff operate it), and the cash rule covers one gate rather than the whole park. All four are fixed, the fifth point (date the platform prices) was applied, and the sixth restated a disclosure line that had already been rewritten. Claude Code also cut its own unsourced 台灣讀者比較常用的. Build passed at 58 articles with the same 25 style reminders and none on this page. Production verified: HTTP 200, exactly one anchor carrying aff_adid 1443707 with rel sponsored noreferrer noopener and target _blank, the affiliate notice now renders on this page with the same counts as the 青之洞窟 page, asoview and 東第 2 閘門 and 由工作人員操作 all present, 掃 QR code and 差額很小 absent, and at 390px the page does not scroll sideways with the link ending at x=356. **Why this page:** a full backend read on 2026-09-21 found 853 active users in 28 days producing one `affiliate_click` (2 events) and zero orders, while five of the ten highest-traffic pages carried no affiliate link at all. This is the single-variable test of whether the missing exit is the cause. **Do not touch this page before 2026-10-05**, then read GA4 `affiliate_click` and Klook Ad ID 1443707 together. If clicks move, repeat on 嬰兒用品採買 and 國際通; if they do not, the problem is query intent and long-tail information articles are the ceiling.

- 2026-09-21 ACTIVE OWNER: Claude Code, claimed before editing. A full read of Search Console, GA4, all three affiliate backends and Kit found the real bottleneck: in 28 days 853 active users produced **one** `affiliate_click` (2 events) and zero orders, while five of the ten highest-traffic pages carry no affiliate link at all. wei96 chose the first test — give `okinawa-zoo-museum-family-guide` (52 clicks in 28 days, intent-clear queries) one ticket link and measure whether `affiliate_click` moves. This is a single-variable test: one link, one page, no other page touched. Files claimed: `content/blog/okinawa-zoo-museum-family-guide.md` and this handoff. **Codex: please do not add affiliate links to other pages while this runs.** Verified today at source: okzm.jp/fee still says 16才以上 1000円 / 15才以下無料 (a Klook reviewer claiming 900 is wrong), the park itself advertises 事前購入セット券なら最大50円お得, 入園料 at 東第2ゲート is cash only, and ベビーカーレンタル is 500円. Klook activity 41471 sells it at NT$182 (retail NT$202, 4.8/666 reviews, 40K+ booked); KKday sells the same ticket at NT$181 with free cancellation a day ahead but only 600+ booked. The link goes to Klook because that is the only one of the two this site has an affiliate account with, and the article says so. Read the result no earlier than 2026-10-05 in GA4 `affiliate_click` and the Klook backend. Nothing else on the page changes, so the reading stays clean.

- 2026-09-21 沖繩包車 guide published (Claude Code; claim released, no active owner). Work commit `aae47d8` adds `content/blog/okinawa-charter-car-family-guide.md` and its cover `public/images/articles/okinawa-charter-car-family-guide/charter-car-cover.webp`. wei96 approved the finished version in chat after seeing the article and the cover. Wording: all eleven speak-human-tw edits were applied in `e82d675`, bold cut from 17 to 4; three of them were accuracy fixes (the late-night surcharge is one operator's rule, 多半寫明 was two Klook product pages, 多數業者 was three operators). The 萬座毛 6h/7h tier was re-read at https://www.1st-taxi.net/price/ on 2026-09-21 and added to the research note's cross-check table, which had been missing it. Cover: Codex generated three candidates under `-s workspace-write` (the `--approve-for-me` launch was refused by Claude Code's own safety classifier) and chose candidate 1; Claude Code then verified it independently at 3x and 7x — the rear seat really holds a five-point child seat, and the white marks on the side sill are specular highlights, not lettering; no text, plate or logo anywhere, no faces to camera. The coverAlt was rewritten because the picture shows the seat already fitted rather than being installed. Affiliate: one Klook text link, Ad ID `1443595` (aid 128859, activity 175908, labels `blog_charter_car_family` / `charter_car_kids_seat`), created by Claude Code in Chrome after wei96 logged in; all five fields were checked programmatically before saving and every character of the generated URL was read back at zoom; not test-clicked. Build passed at 58 articles with the 25 existing style reminders unchanged. Production verified: HTTP 200, title and canonical correct, cover 197,076 bytes served as image/webp and rendering, the Klook link carries `rel="sponsored noreferrer noopener"` and `target="_blank"`, the affiliate notice is identical to the 青之洞窟 page, no reviewer notes or placeholders, sitemap lists the URL; at 390px the page does not scroll sideways and all four tables scroll inside their own 348px containers. **Open:** the Search Console indexing request has not been made — Claude Code cannot drive that form and could not launch Codex with browser approval, so it goes to wei96 or to a Codex run started by wei96. Inbound internal links wait as before (lodging hub 9/24, car hire 9/22).

- 2026-09-21 ACTIVE OWNER: Claude Code, claimed before editing. wei96 chose all 11 speak-human-tw edits on the 沖繩包車 draft, set the publication date to 2026-09-21 (the draft had 2026-09-18), and asked for the cover image to be made by Codex. Files claimed: `100_Todo/drafts/articles/2026-09-18_okinawa-charter-car-family-guide.md`, the cover `public/images/articles/okinawa-charter-car-family-guide/charter-car-cover.webp`, and this handoff. **Codex: please do not start a 包車 page in parallel.** Three of the eleven edits are accuracy fixes, not style: the late-night surcharge was stated as a general rule when only 美ら海 was checked, 多半寫明 covered two Klook product pages, and 多數業者 covered three operators. After the wording pass: cover image (Codex, no text anywhere in the frame), then the Klook tracked link for activity 175908 (Codex, never test-clicked), then wei96 sees the final version before anything reaches `content/blog/`.

- 2026-09-17 ACTIVE OWNER: Claude Code, claimed before starting. Objective: go/no-go on a 沖繩包車 (chartered car with driver) family guide, the next topic on the exposure-and-affiliate list; Keyword Planner on 9/14 put 沖繩包車 at 1000–1萬 with high ad competition (bid to $27.55), and it serves readers of the no-car guide. Every fact goes into the research note as 已交叉比對／單一來源／無法證實 under the new cross-check rule. **Codex: please do not start a 包車 page in parallel.** Files claimed: `research/charter-car-family-2026-09-17.md`, draft `100_Todo/drafts/articles/2026-09-18_okinawa-charter-car-family-guide.md`. 2026-09-18: draft written and reviewed, waiting for wei96 to approve the article before the cover image and the Klook tracked link are made. Codex read-only review returned 15 points; 14 were checked against the original sources and applied, and one was rejected as unverifiable (Codex claimed SUI TAXI caps child seats at two and does not guarantee them; the whole page says only that they are free with advance booking). wei96 confirmed the go on 2026-09-18; the affiliate call is option 1 (keep the Klook link, but teach readers to check the operator is a licensed green-plate business, and say plainly that this site cannot vet every supplier on a platform).

- 2026-09-17 Trip.com record D19827215 corrected (Claude Code decided, wei96 delegated the call). The dashboard record meant for Vessel had been storing the Beach Tower URL and label since Codex created the links on 9/15; the live article link was always correct. Codex updated only that record; Claude Code then verified it independently with zoomed screenshots: D19827215 now stores the Vessel URL and `blog_american_village_vessel`, and its generated link is identical to the one in the article (same Allianceid, SID, trip_sub1, trip_sub3), so the article was not touched. D19827250 (Beach Tower) and D19827285 (Hilton) are unchanged, with last-updated times still 2026-09-15. D19827096 remains unused and was deliberately not deleted.

- 2026-09-16 cross-check corrections live (Claude Code; claim released, no active owner). wei96 asked whether Codex's output had been verified and then asked for every fact to be checked against more sources. Work commit `e14eb90`. Verified by Claude Code itself: Search Console shows all three new guides (no-car, 美國村, 青之洞窟) indexed; the three live Trip.com links carry the right hotel URL, trip_sub1 and trip_sub3. **Open for wei96:** the Trip.com dashboard record D19827215 (meant to be Vessel) stores the Beach Tower URL and label — the live link is correct, only the record is wrong; D19827096 also shows Beach Tower. Corrections on 美國村: Calif Kitchen is on Seaside 3F (Okinawa Navi), so Codex's 9/15 split into two places was wrong and is reverted; both Ryukyu Bus and Okinawa Bus require folding strollers (Codex had said Okinawa Bus was discretionary), Okinawa Bus also says to hold the child; the holiday family fare also covers the last day of Obon and excludes limousine buses; the pool charges are per two hours, children's rate is 幼児・小学生 (not from age 1), and the three numbered-ticket sessions capped at 200 apply only on weekends and holidays (town PDF, 2025-07-01); illumination dates vary by year; Beach Tower parking is 1,000 yen a night (JTB, Hankyu, ANA); shop hours follow Depot Island's official FAQ. 青之洞窟: rip-current advice now includes floating and waiting for rescue (Coast Guard). No-car guide: every checked fact held. Cross-check tables with sources are appended to both research notes. A bounded Codex review of the diff returned 4 points, each checked against the source and applied. Build 57 articles, 89 pages, 25 style reminders; production verified.
...
- 2026-09-16 ACTIVE OWNER: Claude Code, claimed before editing; wei96 said 開始 to adding inbound internal links to the new 青之洞窟 guide, because exposure and affiliate income are the priority. Scope: one sentence with one internal link in each of three live pages that are not under any freeze or experiment in `content/seo-scorecard.md` — `content/blog/okinawa-family-beach-guide.md` (相關文章 paragraph), `content/blog/okinawa-family-beach-packing-list.md` (after the 還沒決定海灘 sentence) and `content/blog/onna-family-stay-guide.md` (where 真榮田岬 is mentioned). The `updated` frontmatter on all three is deliberately left unchanged, following the 9/12 lesson that small non-substantive edits should not move dateModified. No affiliate links, titles or descriptions change. A bounded Codex review of the diff runs before push.

- 2026-09-15 青之洞窟 guide published (Claude Code; claim released, no active owner). Work commit `cf8622b` adds `content/blog/okinawa-blue-cave-snorkel-family-guide.md` and its cover `public/images/articles/okinawa-blue-cave-snorkel-family-guide/blue-cave-cover.webp` (Codex-generated, labelled as an AI illustration). Follow-up `2fc014f` replaced the last 走灘頭 in the source list with 從岸邊下水, which the live check caught. Affiliate: one Klook text link, Ad ID `1432786` (aid 128859, activity 147179, labels `blog_blue_cave_snorkel_family` / `blue_cave_kids_snorkel`), created by Claude Code in Chrome because Codex had hit its usage limit; not test-clicked. Build passed at 57 articles, 89 pages, 25 existing style reminders (unchanged). Production verified: HTTP 200, title and canonical correct, cover image/webp 465 KB and rendered, the Klook link carries rel sponsored and opens in a new tab, the affiliate notice counts match older affiliate pages, NT$1,439 / 118 / 最低參團年齡 / 水上安全條例 / 山田珊瑚農場 present, no reviewer notes or placeholders, sitemap lists the URL; at 390px nothing overflows and the three tables scroll in their own containers. Search Console: indexing requested once on 2026-09-16 at about 22:23 by Codex in wei96's Chrome, after wei96 said Codex was usable again (its quota came back before the 9/19 date in the limit message). Before the request the inspection said 網址不在 Google 服務中; the confirmation read 已要求建立索引。已將網址加入優先檢索佇列。Do not resubmit. wei96 chose to publish now rather than wait for 9/21 because exposure and affiliate income are the priority. Inbound internal links wait for freezes as before (lodging hub 9/24).

- 2026-09-15 青之洞窟 publication claimed (Claude Code). wei96 approved the reviewed draft ("可以 都改"), chose both speak-human-tw edits (applied: 很實在 removed, bold cut from 17 to 5), kept title option 1, and then said the priority is exposure and affiliate income, so the article goes out as soon as its Klook link exists instead of waiting for 9/21. The two-a-week pace in `AI_COLLABORATION.md` is a recommendation; the quality gates (research, Codex review, owner approval) are all met. Files claimed for the publish commit: `content/blog/okinawa-blue-cave-snorkel-family-guide.md` (new), `public/images/articles/okinawa-blue-cave-snorkel-family-guide/blue-cave-cover.webp` (new), the draft, and this handoff. Codex hit its usage limit (until 2026-09-19 18:55) before creating anything, so Claude Code created the Klook link in Chrome: Ad ID `1432786`, aid `128859`, activity 147179, labels `blog_blue_cave_snorkel_family` / `blue_cave_kids_snorkel`, verified by reloading the ad page; not test-clicked. With Codex out, the Search Console request goes to wei96 by hand.

- 2026-09-15 青之洞窟 draft second-reviewed (Claude Code, claim held; nothing published). Codex (read-only) returned 修改後可發布 with 13 points; each was checked against its source before acting, and four needed pages Claude Code had not read yet: Best Dive terms (normal bookings refundable if cancelled by 12:00 three days before; a move to the nearby Yamada coral farm is not grounds to cancel; typhoon or severe sea is a full refund), the Coast Guard rip-current page (do not swim against it, swim parallel to shore first), the Coast Guard hypothermia note (it names infants and older people, so the draft says 嬰兒 not 幼兒), and the Okinawa police water-safety page (2025: 114 water accidents, 51 deaths, provisional; snorkelling and diving operators must register under the 水上安全條例 and the police publish the list). All 13 were applied, plus two errors Claude Code found itself: Pink Mermaid on Klook is one price per person, not dearer for children, and Gigi meets at 前兼久漁港, not a shop. Record in `research/blue-cave-snorkel-family-2026-09-15.md`. Title left at option 1 because it carries both 1000–1萬 terms (沖繩浮潛, 青之洞窟); Codex preferred option 2, so wei96 decides. Next: wei96 reviews the draft, cover and a two-item speak-human-tw list; publication no earlier than 9/21; the Klook link (activity 147179) is generated by Codex only after approval and never test-clicked.

- 2026-09-15 ACTIVE OWNER: Claude Code, claimed before starting; wei96 said 開始 after the next-topic recommendation in chat. Objective: go/no-go on a 青之洞窟 snorkelling-with-children guide and, if it holds, a draft for wei96's review. Why: Keyword Planner on 9/14 put 青之洞窟 and 沖繩浮潛 at 1000–1萬 a month each with high ad competition and bids to about $20, Klook sells these tours, and the site has no page on either. The 美國村 lesson applies first: re-check volume with the 沖繩 prefix, because Italy's Blue Grotto (藍洞) could be inflating it. Pace: the site's two-new-articles-a-week limit is used by 9/14 and 9/15, so nothing is published before 9/21. Children in the sea is safety content: age limits, sea-condition cancellations and life-jacket rules come from operators and official sources only, and the draft goes through Codex's second review. **Codex: please do not start a 青之洞窟 or snorkelling page in parallel.** Files claimed: research note `research/blue-cave-snorkel-family-2026-09-15.md`; if the go holds, draft `100_Todo/drafts/articles/2026-09-15_okinawa-blue-cave-snorkel-family-guide.md`. No frozen page will be edited. **Go held the same day**: 青之洞窟 is Okinawa, not Italy (卡布里島藍洞 is only 100–1000), and although the head-term SERPs are mostly operators and OTAs, personal blogs rank for 沖繩浮潛 and page one for 小孩幾歲可以浮潛 is operator pages plus parents asking on Facebook, Threads and PTT. Draft and research note are written; the cover `public/images/articles/okinawa-blue-cave-snorkel-family-guide/blue-cave-cover.webp` (Codex candidate 1: child in a child-size life jacket beside an adult on a boat, checked at 2x for pseudo-text) is also claimed and stays uncommitted until wei96 approves publication. Codex second review is running. The draft compares three operators (Pink Mermaid via Klook from age 1, Gigi from 2, Best Dive from 4); Best Dive cancellation terms were not read, so the draft makes no claim about them.

- 2026-09-15 美國村 guide published (Claude Code; claim released, no active owner). Work commit `15f33bf` adds `content/blog/okinawa-american-village-family-guide.md` and its cover `public/images/articles/okinawa-american-village-family-guide/american-village-cover.webp` (Codex-generated, labelled as an AI illustration in caption and credit). wei96 approved publication on 2026-09-15 after reviewing the Codex-reviewed version, picked all four speak-human-tw edits, and asked why the hotel names were in English, so the three hotels now lead with their Trip.com Taiwan names: Vessel 花苑沖繩飯店, 北谷沖繩海灘塔飯店, 沖繩北谷希爾頓度假飯店. Affiliate: three Trip.com hotel links, `trip_sub1` `blog_american_village_vessel` / `blog_american_village_beach_tower` / `blog_american_village_hilton`, Ad IDs `D19827215` / `D19827250` / `D19827285`, none test-clicked, so their reports start clean. Claude Code could not fill the dashboard's link form this time (form_input produced a malformed link, `D19827096`, which is unused and left in the dashboard; keyboard input then never reached the fields), so wei96 asked Codex, which created all three in about six minutes via `codex exec --approve-for-me`. For future link or index-request forms, go to Codex first. Build passed at 56 articles, 88 pages, 25 existing style reminders (unchanged). Production verified after the push: HTTP 200, title and canonical correct, cover image/webp 299 KB, each Ad ID present, three `rel="sponsored"` links, the affiliate notice counts match older affiliate pages, 18:47 / 19:02 / 17:45 present, no reviewer notes, placeholders or 觀光情報中心 left, and the sitemap lists the URL. Search Console: indexing requested once on 2026-09-15 at about 21:32 by Codex in wei96's Chrome, after wei96 approved publication; before the request the inspection said 網址不在 Google 服務中 (Google 無法辨識的網址), and the confirmation read 已要求建立索引 / 已將網址加入優先檢索佇列. Do not resubmit. Read `blog_american_village_*` with the other Trip.com entry points; lodging queries are the reason this page has hotel links, and the lodging hub's own read is 9/24. Inbound internal links wait for freezes: lodging hub 9/24, car hire 9/22.

- 2026-09-15 publication claimed (Claude Code). wei96 approved publication on 2026-09-15 ("可以") and chose all four speak-human-tw wording edits, which were applied to the draft before publishing. Files claimed for the publish commit: `content/blog/okinawa-american-village-family-guide.md` (new), `public/images/articles/okinawa-american-village-family-guide/american-village-cover.webp` (new), the draft, and this handoff. Three Trip.com hotel links are being generated in the dashboard with `trip_sub1` `blog_american_village_vessel`, `blog_american_village_beach_tower` and `blog_american_village_hilton`; none will be test-clicked.

- 2026-09-15 美國村 draft finished and second-reviewed (Claude Code, claim held; nothing published, nothing in `content/blog/`). **Correction to the claim below:** the 1萬–10萬 quoted for 美國村 includes Osaka's America-mura. Searched separately, 沖繩美國村 is 1000–1萬 and so is 大阪美國村, and Osaka also shows on page one of the SERP. What keeps the topic worth writing is 美國村住宿 at 1000–1萬 with high ad competition and bids up to $49.55, plus 美國村停車 at 100–1000 where a Facebook group thread on page one is still asking where to park; 美國村親子 is only 10–100, so 親子 stays an angle, not the head term. Draft `100_Todo/drafts/articles/2026-09-15_okinawa-american-village-family-guide.md` targets 沖繩美國村 + 可以玩多久 + 停車 + 住宿. Facts worth reusing, all from official pages and recorded with links in `research/american-village-family-2026-09-15.md`: the last route-120 bus to Naha Airport leaves 美浜アメリカンビレッジ入口 at 18:47 on weekdays and Sundays but 19:02 on Saturdays and holidays, with 12/31–1/3 and Obon on the Sunday timetable; the tourist information centre rents strollers only until 17:45, before summer sunsets; the Ferris wheel was demolished in 2022 and the GOO Chatan direct bus stopped on 2025-02-02, though older posts still show both; Hilton Chatan has charged staying guests 500 yen a night to park since 2026-05-15 while its FAQ still says free. Cover: Codex generated three; candidate 1 was rejected for pseudo-lettering on an awning found at 2x, candidate 3 (boardwalk, family from behind, no Ferris wheel) was cropped to 1600×900 WebP at `public/images/articles/okinawa-american-village-family-guide/american-village-cover.webp` and stays uncommitted. Codex second review (read-only): 修改後可發布, 12 points, each checked against its source before acting — 10 applied, the pool's accompaniment age removed rather than kept because it was read only on a machine-translated page, and the last is the publication step; table in the research note. Three Trip.com hotel links are placeholders; tracking IDs are generated only after approval and will not be test-clicked. Next: wei96 reviews this exact version plus a four-item speak-human-tw list. Inbound internal links wait for their freezes (lodging hub 9/24).

- 2026-09-15 ACTIVE OWNER: Claude Code, claimed before starting. wei96 delegated the choice ("你來定") from a ten-topic Keyword Planner shortlist given in chat on 9/14. Objective: go/no-go on a 美國村 (Mihama American Village, Chatan) family guide and, only if it holds, a draft. Why this topic: Keyword Planner (Taiwan, 2025/9–2026/8, range values) puts 美國村 at 1萬–10萬 a month with low ad competition, and the site mentions it only in passing inside four itinerary articles; there is also no Chatan lodging page, while Naha, Onna, the south and Motobu each have one. Other shortlisted volumes from the same run, for whoever picks next: 瀨長島, 萬座毛, 首里城 and 波上宮 1萬–10萬; 青之洞窟, 沖繩浮潛, 沖繩包車 and 沖繩esim 1000–1萬 with high ad competition (esim bid up to $73). **Codex: please do not start a 美國村 or Chatan page in parallel.** Next step before any drafting: Search Console queries containing 美國村 or 北谷, then the SERP; if the top ten leave no room for a personal family blog, stop and report rather than write. Files claimed: research note `research/american-village-family-2026-09-15.md`; after the SERP check passed the same day, and with wei96 asking for the research, draft, cover and Codex review all to be finished before showing, also the draft `100_Todo/drafts/articles/2026-09-15_okinawa-american-village-family-guide.md` and the cover `public/images/articles/okinawa-american-village-family-guide/american-village-cover.webp` (kept uncommitted until wei96 approves publication). Nothing goes into `content/blog/`. No frozen page will be edited (car hire, souvenir pages and English car seat until 9/22, lodging hub until 9/24). A draft, if wei96 approves writing one, goes to `100_Todo/drafts/articles/` and through wei96's review and Codex's second review before anything is published.

- 2026-09-13 article files claimed before creation (Claude Code, claim held): `content/blog/okinawa-no-car-family-guide.md` and `public/images/articles/okinawa-no-car-family-guide/no-car-cover.webp`. They are placed in the working tree only to build and preview, and **stay uncommitted until wei96 says the page may be published** — any session finding them there should leave them alone. Cover: candidate 2 of the three Codex generated (hibiscus in the foreground, a stone wall, the bus fully out of focus). All three were inspected at 3x on the bus front first; candidate 3 showed a pseudo number plate and an emblem, which is the fake-text risk an AI cover must avoid. Klook link generated: ad ID `1429338`, activity 19157, not test-clicked.

- 2026-09-14 no-car guide published (Claude Code; claim released, no active owner). Work commit `3b09e87` adds `content/blog/okinawa-no-car-family-guide.md` and its cover `public/images/articles/okinawa-no-car-family-guide/no-car-cover.webp`. The cover was generated by Codex's image tool at wei96's request, cropped to 1600×900 WebP, and labelled as an AI illustration in both caption and credit, following the 9/3 precedent. wei96 approved publication on 2026-09-14 after reviewing the final version; Codex's two review rounds and every outcome are in `research/no-car-family-guide-2026-09-13.md`. Affiliate: one Klook text link, ad ID `1429338`, pointing to activity 19157, with labels `blog_no_car_family_guide` / `north_day_tour` stored in the Klook backend rather than the URL. It was checked on the generator page and **deliberately not test-clicked**, so its report starts clean. **Correction for the 9/24 read:** the single Trip.com click on `blog_churaumi_family_time` (D19767295) seen on 9/13 is very likely Claude Code's own verification click on 2026-09-10 around 23:10, made when that link was created. Discount it; the entry-point experiment has no confirmed reader click yet. Production: verified about 45 seconds after the push: HTTP 200, title and canonical correct, cover served as image/webp at 313 KB, the Klook link carries aff_adid 1429338 with rel sponsored, the 17:13 last bus and the Igei rest stop are present while 16:15 is absent, and the sitemap lists the URL. The affiliate notice is in the server HTML exactly as on the older affiliate pages, with identical counts for 合作說明 and the /affiliate-disclosure link; a literal search for 本文含合作連結 finds nothing on any of them because the phrase is split across elements, so that check is useless and should not be read as a missing notice. Search Console: indexing was requested once on 2026-09-14 at about 21:36, by Codex driving Chrome through its browser_use feature, after wei96 asked for it in chat; before the request the inspection said the URL was not on Google, and Search Console confirmed with 已要求建立索引 and 已將網址加入優先檢索佇列. Do not resubmit. Claude Code could not submit this form by automation in four attempts; Codex first stopped to ask for confirmation, and finished once its session was resumed by id with the owner approval stated. Follow-up `22c1447` replaced 伊芸 with 伊藝 in the article, draft and research note, because 芸 is the Japanese form and reads as Simplified Chinese to a reader in Taiwan; Japanese quotations keep the original. Verified live: 伊藝 present, 伊芸 absent. Inbound internal links from other pages wait for their freezes to lift: yui-rail around 9/18 (which also needs its free-infant rule corrected), car hire 9/22, lodging 9/24.

- 2026-09-13 Codex review round two, loop closed (Claude Code, claim held). Codex was sent the revised draft plus Claude Code's point-by-point responses and asked to answer the two disagreements. It confirmed six of seven fixes, conceded point 5 — the VISIT OKINAWA FAQ does name strollers, as Claude Code had said — and on point 6 supplied the source Claude Code had not found: the operator's own timetable PDFs list 伊芸サービスエリア着（休憩） and 〃発（発着時間は目安です）. That was checked before acting. Neither PDF could be rendered (no poppler) and the browser pane treated both URLs as downloads, showing wei96 two save dialogs, which were cancelled; WebFetch saved the files instead, and a small extractor that inflates the streams and applies the ToUnicode CMaps recovered the text. Both directions stop at Igei for about ten minutes. The same extraction settled the last bus a second time: in the operator's PDF, times follow their stop's label, which the Naha Airport row confirms by ending on its known 19:45, so ロイヤルビューホテル美ら海 ends on 17:13 and 記念公園前 on 17:15; a stray 17:11 belongs to the Orion stop before them. The operator's timetable is valid from 2025-11-01, not the 2026/1/9 printed on kotsu-okinawa.org, so the draft now cites that date. The review loop is closed: every point has a recorded outcome, and the recommendation is that the draft is ready once the cover image exists and the Klook tracking ID is generated.

- 2026-09-13 Codex second review done, and the draft revised (Claude Code, claim held). Codex was invoked directly this time rather than left a note: the CLI is bundled with the desktop app at `%LOCALAPPDATA%\OpenAI\Codex\bin\<hash>\codex.exe` (0.153.4) and is not on PATH, which is why earlier checks concluded it was unavailable. It ran `codex exec -s read-only` from Git Bash (PowerShell 5.1 mangles UTF-8 pipes), reading a prompt placed in the gitignored `output/`. Verdict: 修改後可發布, five must-fix and two suggestions. Each was checked against the source before acting. Agreed and fixed: (1) the last bus is 17:13 from the aquarium stop, not 16:15 — Claude Code's parser printed `slice(0,16)` and cut off the final column, so this was an error in the draft, not in the timetable; the official FAQ also names ロイヤルビューホテル美ら海 as the nearest stop to the aquarium, and the table now uses it; (2) Yui Rail's free-infant rule is per ticket holder, child or adult, up to two preschoolers, the third paying the child fare, with school year rather than age deciding the edge cases; (3) Klook's free under-6 place is seatless, one per adult, and excludes the lunch; (4) "assume no child seat if the page is silent" became "ask the operator"; (7) Onna hotels sit in B and C areas of the airport limousine, not only B. Partly agreed: (5) the stroller sources cover different scopes and are no longer framed as contradicting, but Codex was wrong that the VISIT OKINAWA FAQ speaks only of large items in general — it names strollers explicitly; (6) no standing on the expressway and possible delays are in the official FAQ and were added, while the claimed ten-minute rest stop at Igei could not be verified and was left out. Found separately on re-check: the Klook itinerary does state a 19:00 return to 那霸縣廳前, which the first draft said was missing. wei96 chose title ① and an AI scene cover; a prompt was given for free generation, per the rule against spending waigo credits first. Still pending before publication: the cover image, Klook tracking IDs (generated only after approval), and moving the file into `content/blog/`. `okinawa-yui-rail-family-guide` carries the same simplified infant rule and is left until its observation window ends around 9/18.

- 2026-09-13 no-car draft written (Claude Code, claim held). File: `100_Todo/drafts/articles/2026-09-13_okinawa-no-car-family-guide.md`, outside `content/blog/`, so nothing is built or deployed. It passed the project audit patterns plus the project bans (你家, em dash, mainland-Chinese wording) and a check that every internal link resolves, before commit. The most useful fact it adds to the site: the last Yanbaru Express bus back from the aquarium stop leaves at 17:13 (19:45 at the airport, 2026/1/9 southbound timetable), before the aquarium closes — an earlier version of this entry said 16:15, a parsing error caught by Codex's review, see the entry above, so a same-day bus trip either leaves early or stays a night in Motobu — and two of the three Motobu hotels in `okinawa-churaumi-nearby-family-stay` are stops on that line. Next: wei96 reviews this exact version. **Codex: once wei96 has seen it, please give the second review the collaboration rules require.** The reviewer notes at the end of the draft list what most needs checking, chiefly the child-seat paragraph (YMYL) and the stroller paragraph, where only the 2020 MLIT rule for two-seat strollers is officially confirmed.

- 2026-09-13 drafting approved by wei96 (Claude Code, claim held). Draft file claimed before creation: `100_Todo/drafts/articles/2026-09-13_okinawa-no-car-family-guide.md` (corrected from the path first named here, to match the repo's `drafts/articles/` convention). It stays out of `content/blog/` until wei96 has reviewed the exact version and Codex has given a second review, per the article approval gate. No affiliate link will be generated until the draft is approved, so no orphan tracking IDs are created.

- 2026-09-13 no-car guide research complete (Claude Code, claim still held pending wei96's decision). Full record in `research/no-car-family-guide-2026-09-13.md`. Recommendation to wei96: go, provided the head term is 「沖繩自由行不開車」 and the differentiation is family decisions rather than bus mechanics, where 一直玩的馬摩 is already far more complete. Verified facts worth reusing: the child-seat exemption for buses, taxis and tour buses comes from 道路交通法施行令 第26条の3の2 第3項第6号 (general passenger transport vehicles), pulled from the e-Gov API because the page itself is script-rendered; the Yanbaru Express Bus takes about 2h15–2h20 from Naha Airport to 記念公園前, first airport departure 10:10, 2,000 yen adult and 1,000 yen child, with seat-using children from age 1 charged the child fare (timetable from 2026/1/9 on kotsu-okinawa.org, FAQ on the operator site). One trap: WebFetch summarised that timetable as 20–35 minutes by misaligning columns, so parse the table directly. One consistency issue for later, not fixed now: `okinawa-4-days-family-itinerary` says the highway bus takes about 3 hours. Stroller rules differ between VISIT OKINAWA (fold on buses) and MLIT (unfolded as the basic rule, operator discretion), and an article must present both. No article file created and no page touched.

- 2026-09-13 ACTIVE OWNER: Claude Code, claimed before starting, wei96 approved. Objective: evaluate and, if it holds up, draft proposal #2 from Codex's 9/12 weekly review, the no-rental-car guide. **Codex: please do not start proposal #2 in parallel.** Keyword Planner on 9/12 (Taiwan, range values) moves the head term: 「沖繩自由行不開車」 is 1000–1萬 a month, 「沖繩不自駕」, 「沖繩不開車」 and 「沖繩大眾運輸」 are 100–1000 each, while the proposal's 「不租車」 wording is only 10–100 and 「沖繩親子不租車」 returns no data. So the article targets 不開車, with families as the angle inside it rather than in the head term. What makes it worth writing is the downstream purchase: 「沖繩一日遊」 is 1000–1萬 with high competition and a top-of-page bid up to $16.53. Site Search Console shows zero impressions for 不租/不開 queries in three months, which means no page answers them, not that nobody searches. Next step, before any drafting: check the SERP for 沖繩自由行不開車; if the top ten are all OTAs and operators, stop and report rather than write. Files: research note `research/no-car-family-guide-2026-09-13.md` (claimed 9/13, research and go/no-go only, no article file yet). The SERP check passed on 9/13: the top ten are personal blogs (Mimi韓, 一直玩的馬摩, 揪好玩), media (樂吃購, GOOD LUCK TRIP), a Dcard post and a YouTube video, with only one operator (FunTOUR), and none of them is family-specific while Google's related searches include 沖繩自由行不開車親子 and 沖繩自由行不開車住宿. An article file will be named here only if wei96 approves writing it. No frozen page will be edited — the new article may link out to them only. The draft goes to wei96 and to Codex for a second review before anything is published.

- 2026-09-12 Claude Code done; no active owner. (1) The frontmatter revert shipped as `ee14bf4`: the six pages' `updated` fields are back to their pre-9/11 state, and against the pre-9/11 files the only differences left are the Shopee link URLs. Production checked after deploy: souvenir packing now emits `dateModified` 2026-09-08 and the stroller guide 2026-07-27, so the 9/22 freeze on souvenir packing is intact again. (2) The lodging hub's exact live changes were put in front of wei96 in chat as the full diff against `9455e09` plus the live URL, closing the approval-gate gap from 9/10 after the fact. wei96 reviewed the live page on 2026-09-12 and approved it as it stands, so the approval gate is closed and the page stays untouched through the 9/24 read. (3) Index request NOT done. In wei96's Chrome the Search Console top search box accepts the URL through both typing and `form_input`, but an automated Enter does not submit it, and the `/search-console/inspect?resource_id=…&id=…` deep link returns 404. It was then dropped rather than handed over: the production sitemap already carries <lastmod>2026-09-10</lastmod> for this URL, so Google will recrawl the new title on its own, and a manual request would only have been faster, not necessary. No weekly review was written — Codex's `239e015` covers the week, and every freeze it restates stands.

- 2026-09-12 Claude Code claim, made before editing and released later the same day (see the entry above). Three objectives. (1) Restore the `updated` frontmatter on the six pages touched by the 9/11 Shopee Sub_id backfill to their pre-9/11 state. That backfill was tracking-only and should not have moved `dateModified`, which the article template emits; three pages had no `updated` field before and had one added, and one of the six, `okinawa-souvenir-packing-guide`, is under the 9/22 freeze Codex's review restates. The revert returns that page to exactly the state the freeze recorded (`updated: "2026-09-08"`), and every Shopee link stays tagged. (2) Show wei96 the live lodging hub `first-okinawa-where-to-stay`, which went out on 9/10 without passing the article approval gate in `AI_COLLABORATION.md`. (3) Request indexing once for that URL, whose title changed, so the 9/24 read reflects the new title. Files claimed: `content/blog/okinawa-baby-bottle-hotel-cleaning-guide.md`, `okinawa-family-car-emergency-kit.md`, `okinawa-family-drive-toilet-convenience-store.md`, `okinawa-family-shopping-guide.md`, `okinawa-family-stroller-guide.md`, `okinawa-souvenir-packing-guide.md`, and this handoff. Not touched: Codex's 9/12 review and every other frozen page. No weekly review will be written here — Codex's covers the week.

- 2026-09-12 weekly review (Codex, data-only): completed `100_Todo/plans/2026-09-12-weekly-review.md` for 9/5–9/11. Search Console's latest complete seven-day view is 9/3–9/9 (the normal 2–3 day lag): 156 clicks / 3,981 impressions / 3.9% CTR / average position 6.6, versus 136 / 3,441 / 4.0% / 7.0 in the previous review. Index overview shows 74 indexed and 9 not indexed. GA4's past seven days is 215 active users / 311 views / 213 Organic Search sessions / 4 Organic Social sessions. Two new articles (`okinawa-best-month-family-guide`, `okinawa-flight-with-kids-guide`) and the week’s existing improvements are recorded in the pushed history; this review did not create them. The newest verified affiliate backend read remains 9/10: Trip.com 2 clicks in 7 days and 0 bookings/commission; Klook 6 clicks in 30 days, 0 tickets and TWD 0; Shopee 58 clicks and no conversions in its 7/1–9/9 report. The Shopee portal is logged out today, so no newer figure is inferred. Facebook/Instagram have no verified current-week public main-content or post-level report; the known two own-test reactions remain excluded. `content:audit:strict` passes at 54 articles / 0 critical / 25 existing style reminders, and `git diff --check` passes. Live samples loaded new-article covers; the Trip.com Churaumi link is disclosed, sponsored and opens separately. The attempted 390px browser override still reported a 1280px viewport, so no new mobile pass is claimed. Preserve freezes through 9/22 (car-hire hub, souvenir packing/customs and English car-seat), 9/24 (lodging hub and Churaumi direct-link read), and no earlier than 9/25 (Shopee Sub_id measurement). Proposals only: OTS-typhoon and no-rental-car transport drafts, a narrowly scoped Zoo/CYK old-page update, two unapproved social angles, and the 9/24 no-change Trip.com entry-point read. This completed documentation work owns only this review, this handoff entry and automation memory.


- 2026-09-11 Shopee Sub_id backfill, wei96 approved (Claude Code). All 28 Shopee short links on the site now carry a Sub_id; twelve did not, and those twelve sat on six articles including the two biggest pages on the site — the stroller guide at 45 clicks in 28 days and the souvenir packing guide at 34. Their product links could not be attributed at all, which is the actual reason nobody could say whether product content works here. Products were not changed, only the tracking, so the variable stays single. The new tags use three levels rather than the existing two: `sub1` article camelCase slug, `sub2` placement, `sub3` product. Article-level analysis is unaffected — filter on `sub1` — and product-level attribution comes free. Note that `8V7dmjkyZo` and `7AcACRk7hy` were each shared by two articles, so even a Sub_id would not have separated them; each was split into its own link. Every one of the twelve was verified through the redirect before the markdown was touched, and a full sweep afterwards returns no `----` anywhere. Build passes at 86 pages. Two operational traps are recorded in `research/affiliate/backend-readings-2026-09-10.md`: after `form_input` the first Get Link click does not fire and a second is needed, and generated links must be read from the field by script rather than transcribed from a screenshot. What this unlocks: in two to three weeks the Click Report, filtered by `sub1`, answers whether those 79 clicks a month on the two product pages produce any affiliate clicks at all. That is the cheap version of the "should I build a second Shopee blog" question, and it needs no new site. Worth keeping in view when that data lands: the disclosed queries on both pages are informational rather than transactional — the stroller page's largest is 沖繩租推車, renting in Okinawa rather than buying, and the packing page's are all customs questions — and site-wide only 5 impressions in three months came from queries containing 買.

- 2026-09-10 Churaumi ticket entry point, wei96 approved (Claude Code). `churaumi-aquarium-family-time` takes 20 organic clicks in 28 days and had no way to buy anything on it — only an internal link to `churaumi-aquarium-ticket-price-guide`, which takes too few clicks to reach the top ten. The comparison link stays for readers still choosing a platform; a direct Trip.com ticket link now sits beside it for readers whose dates are fixed. Generated from the backend as Ad ID `D19767295` with `trip_sub1=blog_churaumi_family_time`, so it reads separately from the comparison page's `churaumi-ticket-comparison` (`D18972921`) and from Klook `1362380`; none of those were touched. Verified by walking the link, not by trusting the tool: it lands on the Chinese attraction page (`lang=zh-TW`, h1 沖繩美麗海水族館), and the article template applies `rel="sponsored"`, `target="_blank"` and the disclosure by itself. Two things to know. The backend's link-history table labels this link "English" while the landing page is Chinese, so that column is metadata noise — do not chase it. And Trip.com has moved the attraction to `/travel-guide/attraction/motobu/okinawa-churaumi-aquarium-10559035`; the old `/things-to-do/detail/50899757/` path still redirects there. Read `blog_churaumi_family_time` on 9/24 against a baseline of 2 Trip.com clicks in 7 days site-wide. Keep expectations honest: a ticket is roughly NT$400 a head, so this tests whether the extra hop was costing clicks, not whether tickets can pay. Full record in `research/affiliate/trip-churaumi-deep-link.md`.

- 2026-09-10 correction: Shopee Sub_id is not broken (Claude Code). The entry above claiming 58 clicks with zero attribution proved the mechanism had failed was wrong, and it was overturned the same day. Resolving the short links settles it without clicking any of them: `s.shopee.tw` returns the sub_ids in its 301 Location header, inside `utm_content`, as Sub_id1 through 5 joined by hyphens — so `okinawaSummerFamilyPackingList-table---` is a working tag and `----` is five empty fields, which is exactly the value the Click Report shows. The command is in `research/affiliate/backend-readings-2026-09-10.md` and needs no login. Of the 28 short links on the site, 16 carry a Sub_id and 12 do not, and the 16 are precisely the batch added by `1685851` on 09-07. So the 58 clicks landed on the 12 older untagged links; the tagged ones went live three days ago on four low-traffic articles and have simply not been clicked yet. Two consequences. The real task is not repair but backfill: regenerate the 12 untagged links with Sub_ids, which needs the backend and wei96's judgement on the products, per `content/affiliate-product-workflow.md`. And the earlier conclusion that "痛點＋推薦 has been proven not to work here" does not hold — the properly tracked version of that experiment is three days old with almost no traffic through it, so it has not been tested yet.

- 2026-09-10 affiliate backends, read at last (Claude Code, wei96 logged in). Every handoff since August has recorded the affiliate numbers as "missing, not zero" because Klook was logged out, Trip.com did not resolve and Shopee's conversion report sat behind a CAPTCHA. All three opened today, and the honest answer is that they are zero — lifetime, on every platform. Trip.com: 2 clicks in 7 days, 0 bookings, settled/pending/upcoming commission all USD 0.00. Klook: 6 clicks in 30 days, 0 tickets, balance 0.00 TWD, and all six landed on text link 1362380 while the legacy 1357775 that carried all 8 clicks back in August now shows none. Shopee: 58 clicks, conversion report empty for 2026-07-01 to 09-09, commission bill blank. Two things worth keeping. First, the open question from 2026-08-28 — whether Shopee Sub_id actually carries — now has an answer, and it is no: 58 clicks, zero attributed, while Trip.com's trip_sub1 comes through correctly on both ads, so the fault is Shopee's line rather than the method. Second, those 58 Shopee clicks are probably not readers at all: nine of them fall inside four minutes on 9/9 and seven inside ten minutes on 9/8, and the referrer column is almost entirely "Others" with one from inside Shopee itself. GA4's 10 affiliate_click per 28 days matches Klook plus Trip.com and not Shopee, which supports the same reading. The backend URLs nobody had written down are now recorded, along with the full readings, in `research/affiliate/backend-readings-2026-09-10.md`; Trip.com's is `https://www.trip.com/partners/dashboard` and neither `alliance.trip.com` nor `affiliate.trip.com` exists, so stop trying them. What this changes: the affiliate problem is not CTA wording or link placement. 452 site clicks in 28 days produce single-digit affiliate clicks and no bookings, because the traffic sits on anxiety queries with no purchase moment — typhoon, toilets on Kokusai-dori, a children's park whose ticket is 1,000 yen and free under 16 — while the pages with real purchase intent get almost nothing. Lodging, the highest-value inventory on the site, took 11 clicks in three months. That is the gap the lodging hub is aimed at, so read Trip.com's click count again on 9/24 alongside it. Do not add Shopee content or links until its Sub_id works; nothing was changed on any page today.

- 2026-09-10 (Claude Code). Rebuilt `first-okinawa-where-to-stay` into the lodging hub the cluster never had, after pulling the Search Console numbers the scorecard had been missing since 2026-07-30. Those numbers changed the plan twice. First, an earlier claim that the site has no head-term hub page was wrong, though for the wrong reason: `okinawa-rental-car-family-guide` had been turned into one the night before by the 2026-09-09 session, not since its 2026-07-03 creation date, so car hire needed nothing. Second, the lodging cluster turned out to be almost invisible — 11 clicks and 258 impressions across three months, about 1.4% of the site's 18.1k, with `how-to-choose-family-hotel` and `south-okinawa-family-stay` at zero impressions and every query containing 住 sitting at average position 87. That is an impressions ceiling, not a CTR problem, which is what scorecard finding C already said. Because the page had almost no query surface to protect (three disclosed queries, three impressions), the title was changed to `沖繩親子住宿全攻略：北中南怎麼選、住幾晚、要不要換飯店`, matching the naming of the existing car hire hub; the URL and slug are unchanged so ranking history is kept. Added a four-region comparison table with columns ordered by decision value so mobile readers see 建議住幾晚 before scrolling, plus the three internal links the page was missing (`okinawa-change-hotels-family`, `south-okinawa-family-stay`, `how-to-choose-family-hotel`), each placed where the reader is making that decision. `lib/reading-paths.ts` gained an entry for this slug, which it had never had despite the page having the most inbound internal links of any lodging article. Verified at 375px on a real dev server: no horizontal overflow, the table scrolls inside its own container, the reading path renders. Audit stayed at 0 critical and 25 style; `npm run build` passed. Deliberately not done, so the structural change stays the only variable: no affiliate links added anywhere, the four existing Trip.com links on this page untouched, and the three zero-affiliate lodging articles left for the next round. Cooling period to 2026-09-24; judge it on impressions and whether any 住宿 query leaves position 80+, not on clicks. One thing worth a look: the English `/en/blog/where-to-stay-in-okinawa-with-kids` outperforms its Chinese counterpart (7/206 vs 5/126), which sits awkwardly with `c8b61eb`'s conclusion that the English SERPs are shut — one topic, small sample, recorded rather than acted on.

- 2026-09-10 sequencing conflict, flagged rather than reverted (Claude Code). The 2026-09-09 handoff (`9455e09`) asks the next session not to open a second hub page before the 9/22 read on the car hire one: "若沒吃到，先查是排名還是內容問題，不要急著開第二個樞紐頁." That commit landed in the working tree after this session had already read `AI_HANDOFF.md`, so the lodging hub above was built without it. Nothing it protects was touched — the four pages it put under a two-week freeze (souvenir packing, customs, English car seat, car hire hub) are all unchanged, and the lodging page measures independently, so no attribution is contaminated. What was ignored is the sequencing argument: wait for evidence the hub tactic works before spending on a second one. The work is not reverted because the URL never changed and reverting is itself a change to measure, but wei96 should decide whether the lodging page stays live through the 9/22 read or gets rolled back. If it stays, the 9/22 review now has two hub tests to read instead of one, on different clusters — car hire against a query pool of 1萬–10萬, lodging against one where the site currently sits at position 87.

- 2026-09-07 (Claude Code). The typhoon funnel was tested end to end rather than inspected, and that caught three faults that would each have made the lead magnet worthless. First, "After confirming redirect to" was still Kit's default `app.kit.com/confirm-subscription`, so a reader who confirmed would have landed on Kit's own site and never received what the form promised. Second, the confirmation email was Kit's English default on an otherwise all-Chinese site. Third — and worth remembering — editing the email body by setting `innerHTML` on the rich text editor showed the new text and returned "Confirmation email updated", but nothing was saved; only real triple-click plus keyboard input persisted. That was found by reading the actual delivered message in Gmail, not by trusting the success toast. Verified path: typhoon article form → submit → delivery page → Chinese confirmation email from hello@okinawafamilynotes.com landing in INBOX → confirm link → delivery page. Kit form is named 沖繩颱風應變卡 (uid `36a789a0af`). All nine test subscribers were deleted, so the list is genuinely zero and every future subscriber is a real reader — wei96 typed the DO IT confirmation since that irreversible step is not something to automate. Watch from 9/11: whether the seven retitled queries take clicks, whether `affiliate_click` moves off 10 per 28 days, and how many real subscribers the typhoon guide's 1,109 impressions convert.

- 2026-09-06 (Claude Code). Kit records 608 visitors to the rental car checklist form and four subscribers, all four of them wei96's own test addresses — no reader has ever signed up, which is why `newsletter_signup_submit` never fired. The form works; the offer did not match. `NewsletterSignup` now takes an `offer` key with `NEWSLETTER_OFFERS` holding each form id and its copy, and `newsletterOfferBySlug` in the article template routes the typhoon guide and the two rainy-day articles to the new typhoon action card (Kit form `36a789a0af`, delivery page already live). Adding a third offer needs one entry in each map. Verified live. Car hire affiliate links extended from one article to four — insurance, luggage/stroller and pickup/return joined the child seat guide, each with its own `trip_sub1`; refuelling, parking, driving pace and licence translation were deliberately left out. Images: the site uses plain `<img>` with no Next.js optimisation, so nine PNG covers of 1.8-2.6 MB and four 2880x3840 photographs were resized to 1600px long edge and converted to WebP at quality 92, taking the site from 34.3 MB to 11.4 MB and Gyokusendo from 6.7 MB to 2.4 MB; originals stay in place because `scripts/render-daily-carousels.ps1` and several social render scripts read them by path. Dropped from the backlog: reshaping the four portrait covers to 16:9 — `ArticleCard` already normalises with `aspect-[16/10]` and `object-cover`, so the problem did not exist. Open: 9/11 check on whether the seven retitled queries (Gyokusendo, Naha luggage, monorail, child seats, zoo, DMM) start taking clicks and whether `affiliate_click` moves off 10 per 28 days; Kit still has no domain authentication records in DNS; Zoho free has no forwarding, so replies need the mobile app.

- 2026-09-05 weekly review (Codex, data-only): completed `100_Todo/plans/2026-09-05-weekly-review.md` for 2026-08-29～09-04 in commit `3acebfb` (`Record September weekly review`). Search Console's last complete seven-day range (8/27～9/2) is 136 clicks / 3,441 impressions / 4.0% CTR / average position 7; GA4 (8/29～9/4) is 169 active users, 273 views and 156 Organic Search sessions. Index overview, last updated 8/28, shows 71 indexed and 9 not indexed. Three user-approved articles and their one-time indexing requests were recorded in the existing handoff; no new public content, deployment, index request, social posting or affiliate placement was made by this review. Facebook/Instagram have no new public main content or post-level report for the interval. Shopee has no current report (last verified Click Report was 0; Conversion Report CAPTCHA is not bypassed); Klook and Trip.com platform reports remain unavailable, so clicks/orders/commission are missing, not zero. `content:audit:strict` passed with 52 articles, 0 critical issues and 24 existing style reminders; `git diff --check` passed. The live child-sick-guide sample loaded its image and article content, but a real 390px browser width was not available, so no fresh mobile-image pass is claimed. Proposed only: OTS-typhoon and central-rain new-article briefs, a non-cooling stroller-guide decision table, two unapproved social concepts, and 14-day no-change Klook control measurement. This documentation work owns only the weekly report and this handoff entry; both are now complete.


- 2026-09-03/04 content and SEO work (Claude Code, temporary handover; Codex's morning schedule has been stopped by wei96). Published three articles with wei96's approval: `okinawa-souvenir-customs-taiwan` (which Okinawa souvenirs Taiwan prohibits, sourced from the Bureau of Animal and Plant Health Inspection and Quarantine), `okinawa-child-sick-injured-guide` (leads with おきなわ #7119, the prefecture's 24-hour multilingual emergency consultation running since 2024-09-01, plus three-tier triage, a Japanese symptom table and the NHI reimbursement requirements) and `okinawa-baby-supplies-shopping-guide` (アカチャンホンポ has three Okinawa stores but none in Naha). Site is now 52 articles. Rewrote four titles to lead with the answer the query asks for — Gyokusendo, Naha airport luggage storage, monorail child fare and child seats — after Search Console showed roughly 242 impressions and zero clicks across them at average position 7; no URLs changed. Added the four-company child seat comparison to the car seat article and `research/child-seat-comparison.md`. Created `/checklist/okinawa-typhoon-action-card` (noindex, awaiting a Kit form id before the typhoon article links to it). Published `hello@okinawafamilynotes.com` on both contact pages, replacing the outdated line promising a future address. Marked `lead_magnet_delivery_view` as a GA4 key event; `newsletter_signup_submit` still has never fired, so if it stays absent after a week suspect the Kit embed listener. Requested indexing for the three article URLs. Covers now use imagery rather than HTML text cards, per wei96's feedback: the store aisle is a Pexels photo with full credit, the other two are AI-generated scene illustrations labelled as such in caption and credit. Kit remains on the Free plan (unlimited forms, but no POP, IMAP or forwarding), and DNS carries no Kit authentication records. Open items: unify the nineteen portrait images (several serve as covers, which breaks the article grid and OG cropping) and compress four 2880x3840 originals; second batch of title fixes for the zoo/children's-kingdom cluster, whose impressions are up about 145%.

- 2026-09-01 morning content run (Codex, planning-only; completed): the dated grid ends on 8/15. Although `efe6c3c` adds a source-backed「沖繩伴手禮可以帶回台灣嗎」draft and a search-gap analysis, the draft, any cover/infographic and the proposed title/opening changes remain unapproved; they are not public-content work. The known social pool likewise has no approved, duplicate-cleared source article: Gyokusendo and DMM await existing confirmation, the car-emergency-kit carousel awaits a visual rebuild, and the typhoon topic is paused. Therefore Content A and B made no article, old-page, image, internal-link, affiliate/Klook, social-asset, upload, scheduling or publication change. Full record: `100_Todo/plans/2026-09-01-morning-content-gate.md`. `content:audit:strict` passed (49 articles, 0 critical issues, 24 existing style reminders), `git diff --check` passed, and the approved retry after sandbox `spawn EPERM` passed the production build with 80 static pages. No files remain claimed by this completed gate record; no commit, push, deployment, production verification or Search Console action occurred, and 11:00 remains out of scope.

- 2026-08-31 GA4 key event (Claude Code, authorised by wei96): `lead_magnet_delivery_view` is now marked as a key event on property `a400577057p544938353`. Before this the property had zero key events configured other than the unused default `purchase`, so 「重要活動」 read 0 and no funnel could be measured. `newsletter_signup_submit` could not be marked because GA4 has not yet recorded that event name — `newsletter_signup_view`, `form_start` and `lead_magnet_delivery_view` all appear in 近期事件, submit does not. Measurement only went live on 2026-08-30, so this is expected at one day of data; if view keeps firing and submit stays absent after a week, suspect the Kit embed submit listener rather than reader behaviour. Treat Kit as the source of truth for actual subscriber counts.
- 2026-08-31 repository hygiene (Claude Code, temporary handover while Codex is unavailable): committed the uncommitted working tree in five scoped commits. `components/affiliate-disclosure-notice.tsx` and `lib/affiliate.ts` had never been added to git despite being imported by `app/blog/[slug]/page.tsx`, `app/okinawa-family-travel-gear/page.tsx`, `app/okinawa-family-trip-booking/page.tsx` and `components/markdown-content.tsx`; a clean checkout would not have built. `.gitignore` now excludes `.claude/`, `/output/` and `/outputs/`. Eight unused working images under `public/images/articles/okinawa-rental-car-license-translation/` are deliberately left untracked — the article only references `license-documents-cover.jpg` and `three-documents.jpg`, both already tracked. `content:audit:strict` passed with 49 articles and 0 critical issues (24 style reminders) and `npm run build` passed. No push, deployment or publication occurred. Collaboration rules in `AGENTS.md` and `AI_COLLABORATION.md` were left unchanged so Codex can resume unchanged.

- 2026-08-31 morning content run (Codex, planning-only): the dated master grid still ends at 8/15. The 8/29 weekly review's topic and social ideas remain proposals, not approved drafts; the known candidate pool remains ineligible because of original-photo/rights/person evidence, mobile-material readability, or missing complete-draft/user-approval gates. Therefore neither Content A nor Content B opened an article, old-page edit, image, internal link, affiliate/Klook placement, social asset, upload, schedule or publication. Gyokusendo and DMM remain pending their existing confirmation, the car-emergency-kit carousel awaits a visual rebuild, and the typhoon topic remains paused. Details: `100_Todo/plans/2026-08-31-morning-content-gate.md`. `content:audit:strict` passed with 49 articles and 0 critical issues (24 existing style reminders), `git diff --check` passed, and the approved retry after sandbox `spawn EPERM` passed the production build with 80 static pages. No commit, push, deployment, production verification or Search Console action occurred; 11:00 remains out of scope.

- 2026-08-30 newsletter funnel recovery (Codex, deployed and verified): wei96 explicitly approved both production push and Kit downgrade. Commit `ea1526d` is on `origin/main`; production `/newsletter` exposes the new measurement marker, renders the public Kit form for Form ID `9801697`, and production `/checklist/okinawa-rental-car-documents` returns 200 with its correct canonical and `noindex, nofollow`. Kit confirmed the account is now on Free and restored dashboard access with all 4 subscribers intact. Email sequences redirect to the upgrade page, so the former five-email sequence and Visual Automation are not operational; current MVP is form, double opt-in/checklist delivery, subscriber list and manual Broadcasts. GA4 scripts and the three privacy-safe event implementations are deployed, but no Realtime receipt is claimed because the controlled browser did not expose `window.gtag`; judge receipt from actual GA4 reports. A fresh bounded CC call stalled with no output and was terminated, so the recorded 2026-08-27 CC review remains the available second opinion.

- 2026-08-30 SEO repair release (Codex, deployed and verified): commit `5cb2300` is on `origin/main`. The live `/affiliate-disclosure`, `/newsletter`, budget calculator and social image all return 200; the homepage footer exposes honest newsletter and cooperation labels, and the production sitemap contains both routes. The disclosure canonical and calculator Open Graph/Twitter image were verified on production. Mobile checks passed at 390px with no horizontal overflow. Strict content audit passed with 49 articles and 0 critical issues, `git diff --check` passed, and the production build generated all 80 static pages. No article, social asset, analytics implementation or unrelated dirty file was included.

- 2026-08-30 morning content run (Codex, planning-only): the dated master grid still ends at 8/15. The 8/29 weekly review's OTS-typhoon, central-rain, Naha-airport-luggage and social ideas are explicitly proposals, not approved drafts, and the prior candidate pool remains ineligible because of original-photo/rights/person evidence, mobile-material readability, or missing complete-draft/user-approval gates. Therefore neither Content A nor Content B opened an article, old-page edit, image, internal link, affiliate/Klook placement, social asset, upload, schedule or publication. The topic registry also keeps Gyokusendo and DMM awaiting confirmation, the car-emergency-kit carousel awaiting a visual rebuild, and the typhoon topic paused. Details: `100_Todo/plans/2026-08-30-morning-content-gate.md`. `content:audit:strict` passed with 49 articles and 0 critical issues (24 existing style reminders), `git diff --check` passed, and the approved retry after sandbox `spawn EPERM` passed production build with 80 static pages. No commit, push, deployment, production verification or Search Console action occurred; 11:00 remains out of scope.

- 2026-08-29 weekly review (Codex, data-only): completed `100_Todo/plans/2026-08-29-weekly-review.md` for 8/22–8/28. Search Console is 126 clicks / 3,382 impressions / 3.7% CTR / position 6.9; index overview is 72 indexed and 8 not indexed. GA4 is 159 active users, 240 views and 148 Organic Search sessions (3 Organic Social). No new commit, public article, old-page change, deployment, index request or social publication is verified for the interval. Shopee Click Report is 0; its Conversion Report is CAPTCHA-blocked, Klook is logged out and Trip.com does not resolve, so orders and commissions remain missing—not zero. `content:audit:strict` passed with 49 articles and 0 critical issues (24 existing style reminders); `git diff --check` passed. Live luggage-storage links and loaded images were sampled, but the in-app browser again ignored the requested 390px viewport, so no fresh mobile-image pass is claimed. Preserve the dirty worktree; do not alter cooling pages, the three Klook control IDs, or unapproved social assets.

- 2026-08-29 morning content run (Codex, planning-only): the dated grid in the master schedule ends at 8/15, and the recorded 8/16–8/29 gate still requires the missing 8/22 English/affiliate observation, 8/23 weekly review, 8/24 complete candidate draft and explicit user approval before a candidate can proceed. The known pool remains ineligible: Tondou, Daruma Temple and ASHIBINAA lack original-file, people/privacy or public-use evidence; the Churaumi duration draft fails its mobile-material gate; and the 8/28 newsletter funnel implementation remains awaiting the user's review. The topic registry provides no new approved article for social extension; Gyokusendo and DMM remain pending confirmation, and the car-emergency-kit carousel remains pending a visual rebuild. Therefore no article, old-page edit, image, internal link, affiliate/Klook placement, social asset, upload, schedule or publication was created. Details: `100_Todo/plans/2026-08-29-morning-content-gate.md`. Strict audit passed (49 articles, 0 critical; 24 existing style reminders), `git diff --check` passed, and the approved retry after sandbox `spawn EPERM` passed production build with 80 static pages. No commit, push, deployment, production verification or Search Console action occurred; 11:00 remains out of scope.

- 2026-08-28 newsletter funnel measurement (Codex, local implementation complete; awaiting user approval): the existing Kit form now records `newsletter_signup_view` and `newsletter_signup_submit`; the noindex rental-document delivery page records `lead_magnet_delivery_view`. Event parameters are limited to page path, source path, placement and lead-magnet ID; no Email, subscriber ID, confirmation URL, query string or other personal data is sent. Mobile checks passed at a 375px content width for the article-end form and delivery page with no horizontal overflow; the delivery page keeps its self-canonical and `noindex, nofollow`. The Kit embed loaded on the article-end form. Local Google Tag Manager was unavailable during preview, so a real GA4 event must be verified once on production after deployment; do not claim GA4 receipt yet. `content:audit:strict` passed with 49 articles and 0 critical issues (24 existing style reminders), and production `next build` passed with 80 static pages. Claimed files: `components/newsletter-signup.tsx`, new `components/newsletter-funnel-tracker.tsx`, `app/checklist/okinawa-rental-car-documents/page.tsx`, new `100_Todo/plans/2026-08-28-newsletter-funnel-measurement.md`, and this handoff. No article body, affiliate link, Klook placement or already-dirty `app/blog/[slug]/page.tsx` was changed. Nothing has been committed, pushed or deployed; show the exact mobile preview and obtain user approval first.

- 2026-08-28 morning metrics run (Codex, planning-only): GA4 for 2026-07-31 to 2026-08-27 shows 503 Organic Search sessions, 1,364 `page_view` events and 13 `affiliate_click` events from 4 users. At least one affiliate event is a known site test and GA4 does not distinguish tests from reader clicks, so the 13 is not a natural-conversion count. Kit has 4 confirmed subscribers and 0 new today; the upcoming newsletter events are not yet deployed, so their absence in GA4 cannot prove zero signups. Shopee Click Report is 0 for 8/21–8/27, but conversion data is CAPTCHA-blocked; Klook and Trip.com are logged out. Orders and commission therefore remain missing data, and Klook IDs `1357775`, `1362380`, `1362390` remain untouched controls. No article, image, internal link, affiliate placement, social asset, upload, schedule or publication was created. Details: `100_Todo/plans/2026-08-28-morning-metrics-baseline.md`. Strict audit passed (49 articles, 0 critical; 24 existing style reminders), `git diff --check` passed, and the approved retry after sandbox `spawn EPERM` produced 78 prerendered routes. No commit, push, deployment, production verification or Search Console action occurred; 11:00 remains out of scope.

- 2026-08-27 morning content run (Codex, planning-only): the 8/16–8/29 schedule permits a mobile social preview only after both an approved new-article intent and a passed duplicate check. There is no 8/22 English/affiliate observation record, 8/24 approved complete new-article draft or explicit user approval. The 8/25 OTS-typhoon, central-rain and Gyokusendo items are proposals, not approved candidates. The topic registry also pauses typhoon social content, keeps the Gyokusendo carousel awaiting its prior approval, and records the ASHIBINAA shopping angle as withdrawn for duplication and unresolved photo evidence. Therefore no preview, copy, image, internal link, affiliate/Klook change, upload, schedule or publication was created. Details: `100_Todo/plans/2026-08-27-social-preview-gate.md`. Strict audit passed (49 articles, 0 critical; 24 existing style reminders), `git diff --check` passed, and the retry after an initial sandbox `spawn EPERM` passed production build with 80 static pages. 11:00 deployment and indexing remain out of scope.

- 2026-08-26 morning content run (Codex, planning-only): the 8/16–8/29 plan schedules a preflight only for an already approved candidate. There is still no 8/22 observation record, 8/23 source review, 8/24 complete candidate draft or user approval to preflight. The known pool remains ineligible: Tondou, Daruma Temple and ASHIBINAA lack original-file, people/privacy or public-use evidence, and the Churaumi duration draft fails its 390px mobile-material gate. Therefore no substitute topic, article, image, internal link, affiliate/Klook placement or social asset was created. The complete stop record is `100_Todo/plans/2026-08-26-candidate-preflight.md`. Strict audit passed (49 articles, 0 critical; 24 existing style reminders), `git diff --check` passed, and the approved retry after an initial sandbox `spawn EPERM` passed production build with 79 static pages. No commit, push, deployment, production verification, Search Console action, upload, scheduling or publication occurred.

- 2026-08-25 morning content run (Codex, planning-only): the 8/16–8/29 schedule requires a second candidate to be decided from the 8/22 English/affiliate observation and 8/23 weekly review. Neither record, nor an 8/24 candidate draft, exists in the workspace, so no current Search Console, GA4 or platform evidence can support a ranking. The known candidate pool remains blocked: eight high-demand old pages lack matching rights-cleared original photos; Tondou, Daruma Temple and ASHIBINAA still lack original-file, people/privacy or public-use evidence; and the Churaumi duration draft still fails its 390px mobile-material gate. The safe outcome is no second candidate and no new article, not a similar-intent substitute. Details: `100_Todo/plans/2026-08-25-second-candidate-decision.md`. Strict audit passed (49 articles, 0 critical; 24 existing style reminders), `git diff --check` passed, and production build passed with 79 static pages after the initial sandbox `spawn EPERM` retry. No article, image, affiliate/Klook placement, social asset, upload, schedule, publication, commit, push, deployment or Search Console action occurred.

- 2026-08-20 morning content run (Codex, planning-only): completed the scheduled newsletter rental-document checklist user-flow and measurement review in `100_Todo/plans/2026-08-20-newsletter-user-flow-measurement-check.md`. The current implementation loads Kit's public browser-side embed on `/newsletter` and after Chinese article bodies; the formerly described server route `app/api/subscribe/route.ts` is absent, so no server-side subscription request is made. `/checklist/okinawa-rental-car-documents` remains a noindex subscriber-delivery page. GA4 currently records page views only for this flow; no newsletter-specific event exists. The plan separates newsletter page and checklist views from Kit-confirmed subscriptions, forbids sending email addresses or confirmation URLs to GA4, and records that a browser click/HTTP response is not proof of a confirmed subscription. Strict audit passed (49 articles, 0 critical; 24 existing style reminders), `git diff --check` passed, and the approved retry after a sandbox `spawn EPERM` passed production build with 79 static pages. The actual mobile, confirmation-email, Kit-status and delivery-page flow still requires wei96 to enter a self-controlled test email; no email, test subscription, social asset, affiliate/Klook placement, upload, schedule, publication, commit, push, deployment or Search Console action occurred.

- 2026-08-19 morning content run (Codex, planning-only): 8/17's three new-article candidates remain disqualified by missing original-photo/rights/person evidence, and 8/18's Churaumi draft remains disqualified by unreadable mobile material; therefore no new article was selected or drafted. Completed the scheduled fallback: `100_Todo/plans/2026-08-19-yui-rail-photo-reinforcement-plan.md` provides a non-public, evidence-first plan for `okinawa-yui-rail-family-guide` (28 clicks / 615 impressions in the 8/16 three-month GSC inventory). It verifies the operator's currently available child-ticket, QR pass and elevator guidance, and requires original-file provenance, station/date traceability, person/privacy review and on-site photography-rule confirmation before any image enters `public/` or the article. Existing AI/self-made visuals are explicitly not first-hand evidence. Strict audit passed (49 articles, 0 critical; 24 existing style reminders), `git diff --check` passed, and the approved production build passed with 79 static pages. No public article, image, social asset, affiliate/Klook placement, upload, schedule, publication, commit, push, deployment or Search Console action occurred.

- 2026-08-18 morning content run (Codex, planning-only): completed the scheduled validation of the pending first-screen draft for `churaumi-aquarium-family-time`; details are in `100_Todo/plans/2026-08-18-churaumi-morning-quality-check.md`. Official FAQ still supports the 1.5-hour aquarium average, excludes facility-to-facility movement and suggests half a day for park facilities. For August 2026 the aquarium is open to 21:00, but the dolphin, turtle and manatee facilities close at 19:00, so a late aquarium slot cannot be treated as extra time for all surrounding facilities. Existing article links resolve to five distinct relevant pages and article links retain new-tab behavior. The 390px page has no horizontal overflow, but the existing 1200px SVG scales its 23px card descriptions to roughly 6.4px at a 335px rendered width; the mobile material gate therefore fails. The draft remains `pending-user-review`; do not merge, request approval, commit, push, deploy, index, alter affiliate/Klook placements, or make social assets until a readable responsive table or redesigned mobile graphic passes. Strict audit passed (49 articles, 0 critical; 24 existing style reminders), `git diff --check` passed, and the approved retry after a sandbox `spawn EPERM` passed production build with 79 static pages. No public content, image, social item or affiliate placement changed.

- 2026-08-14 affiliate experiment (Codex, measurement phase complete): commit `7db248b` is on `origin/main`. It restores GA4 when Vercel does not supply `NEXT_PUBLIC_GA_ID` by falling back to the project's public measurement ID `G-Z076ZYK89C`. Strict audit passed with 49 articles and 0 critical issues; the production build passed with 79 static pages. Production HTML now loads the Google tag. A live Klook click from `churaumi-aquarium-ticket-price-guide` opened a separate tab, preserved the article tab and tracking parameters, and GA4 Realtime received `affiliate_click` 1 time. The event detail exposed 14 parameter keys, including `affiliate_network` and `content_group`. The 60-day baseline is in `100_Todo/plans/2026-08-14-affiliate-60-day-baseline.md`. Next phase is observation: collect weekly article views, GA4 affiliate clicks, platform clicks, orders and commissions. Ticket and lodging remain equally important; do not add Agoda, move placements or rewrite CTA text before there is enough baseline data to isolate one change.
...
- 2026-08-17 morning content run (Codex, planning-only): completed the scheduled rights/person/official-fact checklist for Tondou, Daruma Temple and ASHIBINAA in `100_Todo/plans/2026-08-17-photo-rights-facts-checklist.md`. All three remain out of the public candidate pool: the original photo files are absent from the workspace, so location evidence, identifiable-person checks and safe crops cannot be validated; no venue or shop has supplied written approval for using the self-shot photos on the site. Current official facts were rechecked: Tondou's two official sources conflict on closing time (24:30 vs 23:30), so do not hard-code hours; Naha's database supports Daruma Temple's basic location/access but not family facilities or photo rules; ASHIBINAA's official pages support general hours, access and family facilities but its website policy is not a photo-reuse grant. No article, image, affiliate/Klook placement, English page, social asset or publication changed. Strict audit passed (49 articles, 0 critical; 24 existing style reminders), `git diff --check` passed, and the approved retry passed production build with 79 static pages. No commit, push, deployment, indexing, upload, scheduling or publication occurred.

- 2026-08-16 morning content run (Codex, planning-only): cross-checked the current Search Console three-month report (2026-07-08–08-13: 292 clicks / 6,790 impressions / 4.3% CTR / position 7.1) with GA4's latest seven-day 100 Organic Search sessions and each article's inline image inventory. The evidence-first priority pool is eight high-demand pages with zero first-hand images: Yui Rail (28/615), car-seat rental (17/563), zoo/museum (23/446), stroller (32/393), typhoon (43/633), souvenir packing (17/384), Kokusai-dori (19/253) and pickup/return (7/222), where figures are GSC clicks/impressions. None has a matching, traceable original photo cleared for public use; self-made graphics, ticket screenshots and unlocated road/sunset photos are explicitly not first-hand evidence. No article, image, affiliate placement, Klook CTA, English page, social asset or publication changed. The complete matrix and stop conditions are in `100_Todo/plans/2026-08-16-first-hand-evidence-priority.md`; 8/17 should keep the separate rights/person/official-fact checklist for Tondou, Daruma Temple and ASHIBINAA. `content:audit:strict` passed (49 articles, 0 critical; 24 existing style reminders), `git diff --check` passed, and the approved retry after a sandbox `spawn EPERM` passed the production build with 79 static pages. No commit, push, deployment, Search Console index action, upload, scheduling or publication occurred.

- 2026-08-15 morning content run (Codex, planning-only): completed the 8/1–8/14 review and drafted the 8/16–8/29 evidence-first cycle in `100_Todo/plans/2026-08-15-two-week-review-and-next-cycle.md`. Four new articles are live, but only the Kokusai-dori title test is an actually deployed old-page change; the Churaumi first-screen improvement remains a draft and must not be counted as live or as a result. Latest usable 8/1–8/7 data (99 clicks / 2,553 impressions / 3.9% CTR / position 7.0; 145 Organic Search sessions) cannot yet judge the newest 8/13 article. The next cycle reduces output pressure: source-backed, rights-cleared first-hand evidence is the sole content variable; no new article proceeds while photo provenance, venue rules, people/privacy, official facts, internal links or user approval remain unresolved. Keep English unchanged until the 8/22 observation gate; AdSense remains paused after the low-value-content rejection; Klook placement and CTA remain unchanged; `/start` is requirements-only until social-direction data exists. Strict audit passed (49 articles, 0 critical; 24 existing style reminders) and `git diff --check` passed. The restricted build first hit `spawn EPERM`; the approved retry passed with 79 static pages. No commit, push, deployment, Search Console, upload, scheduling or publication occurred.

- 2026-08-15 weekly review (Codex, data-only): recorded `100_Todo/plans/2026-08-15-weekly-review.md` for 8/8–8/14. Direct Search Console data is 70 clicks / 1,952 impressions / 3.6% CTR / position 7.2; index overview is 64 indexed and 10 not indexed. GA4 reports 130 active users, 521 page views and 97 Organic Search sessions. Do not treat the 8 `affiliate_click` events as organic: at least one is the documented Klook measurement test and this GA4 view cannot exclude all own-test traffic. Klook was logged out, Shopee was blocked by browser policy, and Trip.com affiliate DNS did not resolve, so platform clicks, orders and commission remain missing. No Facebook/Instagram main content was published in the interval; unpublished drafts remain unpublished. Current live mobile recheck was blocked by Vercel Security Checkpoint, so no fresh mobile/permission claim was made. Strict audit passed again (49 articles, 0 critical; 24 style reminders). Next candidates are DMM duration and Gyokusendo duration pages, subject to source/rights/user-approval gates; observe Klook placement unchanged.
...
- 2026-08-14 **改走電子報路線，AdSense 暫停申請（wei96 決策，Claude Code 執行）**：確認退件原因後，wei96 決定**不再申請廣告**，把力氣放在電子報與既有的聯盟連結。理由是以目前每天約 7 個自然搜尋訪客推估，AdSense 月收入僅約 1 到 4 美元，而付款門檻是 100 美元。**AdSense 帳戶保留但不送複查**；`app/layout.tsx` 裡的 `adsbygoogle` 指令碼目前仍在載入，尚未移除，這是待辦。**已完成並上線**：（1）Kit 帳號建立完成、Email 已驗證，品牌名稱設為「沖繩親子旅遊筆記」，**未填信用卡**，14 天試用結束後會落到免費方案；（2）Kit 表單「沖繩租車證件檢查表」已發布，**Form ID `9801697`**，表單 UID `03df8acb9a`，提交端點為 `POST https://app.kit.com/forms/9801697/subscriptions`、欄位名 `email_address`，**不需要 API key**；（3）站上新增 `components/newsletter-signup.tsx`（inline 表單，不做彈窗，避免 Google 對行動版干擾式廣告的負面評價）、`app/api/subscribe/route.ts`（伺服器代送，避開 CORS 並讓讀者留在站內）、`app/newsletter/page.tsx`、`app/checklist/okinawa-rental-car-documents/page.tsx`；（4）訂閱表單已放進**所有文章**正文結尾；（5）`app/privacy/page.tsx` 新增「電子報訂閱」章節，載明 Kit 為處理者、不提供第三方、取消方式。**訂閱誘因刻意不用行程表**，因為站上 5 篇行程文已經把內容給完了；改用租車證件檢查表，理由是譯本沒辦好會卡住第一天、行前急迫性最高。行程表保留給之後當付費商品。**誘因是網頁不是 PDF**，因為讀者以手機為主、PDF 中文字型易出問題，需要紙本的人可用瀏覽器列印。檢查表頁為 `noindex, nofollow` 且不進 sitemap，canonical 指向自身。Kit 確認信的主旨與內文已全部改為繁體中文，「確認後導向」設為檢查表網址，**雙重確認（double opt-in）保持開啟，未勾選 auto-confirm**。**尚未做端到端實測**，需要 wei96 用自己的 Email 走一次完整流程確認。6 封行前自動信的大綱在 `100_Todo/drafts/lead-magnets/2026-08-14_newsletter-sequence-outline.md`，內文尚未撰寫。

- 2026-08-14 **三篇文章的重複句型已改寫（wei96 逐條確認後執行）**：全站「不是 A，而是 B」句型原有 37 處、分布 18 篇，古宇利一篇就佔 6 處。Claude Code 列出前三篇共 14 處的逐條改寫建議，wei96 確認後套用 13 處，**古宇利結尾第 147 行刻意保留**，因為一篇出現一次是正常中文，六次才是問題。同時移除「崩潰」與「爆掉」兩個 `AI_COLLABORATION.md` 已點名要避免的詞。三篇共用的「本文不是實住心得，而是依照⋯⋯整理」模板句，語意完整保留、只改斷句，且兩篇改法不同以免模板換位置。全站提醒數 37 → 24。**未改動任何事實、數字、來源連結或判斷。**

- 2026-08-14 **內容第一手程度盤點（Claude Code 量測，供後續決策）**：掃描 49 篇文章結果 —— **內文完全沒有圖片 29 篇（59%）**、引用本地照片檔 13 篇（27%）、標示示意圖／自製／AI 圖 17 篇（35%）、使用 Unsplash 素材 6 篇（12%）、明講「不是實住心得」6 篇（12%），**平均每篇提到「官方」5.1 次**。且那 13 篇要再打折：`churaumi-aquarium-ticket-price-guide` 的 11 張是購票畫面截圖，不是沖繩現場照片；`okinawa-rental-car-license-translation` 的圖片圖說明寫為「本站製作的情境示意」，也不是實拍。真正含第一手旅遊照片的大約只有 DMM 水族館、沖繩世界玉泉洞等少數幾篇。**這個組合（多數文章無圖、重度引用官方資料）正是 Google 判定「內容空洞」的典型輪廓，判斷上比句型重複更接近退件主因。** 但**不可以把「本文不是實住心得」那句刪掉充數** —— 審查員判斷的是整篇內容不是那一句聲明，刪掉等於隱瞞，代價更高。正確做法是把已有的第一手素材（22 張原始照片批次、DMM／玉泉洞／達磨寺／Ashibinaa／Tondou 拉麵）補進**有搜尋流量但目前零圖片**的文章。下一步建議：交叉比對 Search Console 成效資料與這份無圖清單，選 5 到 8 篇優先補。
...
## 2026-08-27 電子報漏斗設定與 CC 第二意見（Codex）

- Active owner：Codex；只認領電子報紀錄檔與本段交接，不碰工作區其他既有變更。
- Kit 序列 `沖繩親子旅遊行前信｜25 天 5 封`（ID `2873685`）已建立，五封後續信皆為 Published；間隔 3／4／5／6／7 天，累積於雙重確認後第 3／7／12／18／25 天送達。
- Visual Automation（ID `2058268`）已建立：起點限定 `沖繩租車證件檢查表`，動作接到上述五封序列，沒有使用 Any form，也未把現有訂閱者倒灌進序列。
- 自動化目前仍為 inactive。切換 Active 屬帳號層級操作，Codex 內建瀏覽器安全機制拒絕代按；需要使用者本人在可見 Kit 頁面手動切換一次。
- 既有雙重確認與檢查表轉址已成功；五封信已檢查主旨、預覽文字、內容、正式間隔、取消訂閱連結、五個目標頁與 UTM，並清除 Kit 預設教學範本殘留。
- 使用者已明確同意把去識別的 GSC 數字、漏斗狀態、內容候選與分潤方向交給 Claude Code 檢討；未提供 Email、帳號或個資。CC 回覆已整理在 `100_Todo/plans/2026-08-27-newsletter-funnel-cc-review.md`。
- 下一步：使用者手動 Active 後，以全新測試信箱走真實表單與雙重確認流程；Day 0 起觀察 14 天。內容優先補強玉泉洞既有決策頁，OTS 颱風租車文同步準備，中部雨天半日文排第三。任何完整文章仍需先給使用者確認，不能直接發布。
...
## 2026-08-25 每週檢討（Codex，資料與交接文件）

- 已完成 8/18～8/24 的證據式週檢討：`100_Todo/plans/2026-08-25-weekly-review.md`。Search Console 為 118 點擊／3,069 曝光／3.8% CTR／平均排名 7.0，索引 72／未索引 8；GA4 為 158 位活躍使用者、199 次瀏覽、146 個 Organic Search 工作階段、2 個 Organic Social 工作階段。
- 本週沒有可驗證的公開新文、舊文補強、部署或社群發布。Shopee Click Report 為 0；Shopee 成交頁 CAPTCHA、Klook 與 Trip.com 後台登出，訂單與佣金均為缺資料，未以 GA4 或站方測試推估。Klook `1357775`、`1362380`、`1362390` 仍是控制組，禁止變更版位或 CTA。
- `content:audit:strict` 通過（49 篇、0 重大問題、24 個既有口吻提醒），`git diff --check` 通過。正式站三頁連結／sponsored 抽查通過；要求的 390px 視窗仍被瀏覽器忽略，故手機圖片複驗未通過、待可套用真實 viewport 時重做。未改冷卻期頁，未發布、提交或推送；既有髒工作區完整保留。

## 2026-09-07 幾月去沖繩（Claude Code）

- Active owner：Claude Code（Codex 暫停期間接手）。
- 新文章 `content/blog/okinawa-best-month-family-guide.md` 已提交並推送（`a2f9943`），正式站 200，封面圖 200（66 KB WebP），首頁列表與 `sitemap.xml` 都有這頁。
- **內容缺口來源**：站上原本沒有任何一篇回答「幾月去沖繩」。數字全部取自氣象廳那霸平年值（1991–2020）。核心論點是一般說法錯了——六月 284.4mm 與九月 275.2mm 幾乎一樣多，而九月還是颱風高峰；七月 188.1mm 反而是夏季最少。
- **開頭刻意先寫但書**：三十年平均不是預測，年降雨 2,161mm、無乾季，任何月份都要留雨天備案。不讓讀者把月均值當保證。
- **內部連結**：從颱風、雨天景點、海灘三篇各加一條入站連結，都插在讀者正在想時間點的那一句旁邊。這是為了避免重演玉泉洞「零入站連結、排名卡在 9」的狀況。
- **封面授權**：Unsplash，作者 Kai Inoue，裁切 1600×900 WebP，已登錄 `content/image-credits.md`。Unsplash 未標示拍攝地點，因此 `coverCaption` 明寫「非沖繩特定地點」，不得改成宣稱是沖繩實景。
- 閘門：`content:audit:strict` 54 篇／0 重大；`next build` 通過、86 個靜態頁。

### 同日社群回文（使用者明確授權 Claude 代為回覆）

- 使用者說明「社團回文你可以回 因為現在暫時由你接手」，本次為該授權下的第一則。**這是暫時性例外，全域 CLAUDE.md 的「不要登入社群網站、按下發布」規則在 Codex 回歸後恢復。**
- 回覆位置：`https://www.threads.com/@am.iuy/post/Dc9mhBok_7L`，原串作者為 `shuge0820`（沖繩在住 17 年、每日天氣帳號），該串 104 次瀏覽。以 `@okinawa.family.notes` 回覆 am.iuy 的「3 月初去沖繩會多雨嗎」。
- 已重新載入頁面確認回覆確實存在，不是只看到「發佈中」提示。
- 內容為平年值數字＋「個人檔案有連結」的軟性指路，未貼站內網址。已確認個人檔案確實掛著 `okinawafamilynotes.com`。
- 發文前修正一個算錯：三月 142.8mm 是十二個月裡**倒數第五**（1 月 101.6、12 月 110.0、2 月 114.5、11 月 119.1 都比它低），草稿原本誤寫倒數第四。
- 帳號現況：2 位粉絲、2,191 次最近瀏覽。8 月交接檔提到的「帳號疑似被限制動作」本次未再出現，回覆正常送出。

### 下一步

- 觀察這篇的 query 是否吃到「沖繩幾月去」「沖繩梅雨」「沖繩颱風季」，以及那則回文是否帶進站流量（GA4 Organic Social）。
- 尚未寫的內容缺口：上網與付錢（`網卡`／`eSIM`／`換匯` 各 0 篇）、母乳與擠奶（0 篇）。

## 2026-09-07 主詞樞紐頁：沖繩租車（Claude Code）

- Active owner：Claude Code。已提交並推送（`c7a3752`），正式站標題已更新，封面圖 200。
- **策略轉向的依據**：同日用 Google Ads 關鍵字規劃工具實測（台灣，2025/08–2026/07）。沖繩租車／自由行／住宿／景點／機票各 **1萬–10萬**；站的定位詞「沖繩親子」只有 **100–1000**。54 篇文章**沒有一篇在承接主詞**。完整報告在 `C:\AI\cc\100_Todo\projects\2026-09-07_育兒採買聯盟站-可行性研究.md`。
- **做法是升級既有的 `okinawa-rental-car-family-guide`，不另開網址。** 那篇本來就是薄的總覽頁（63 行、已連出 6 篇），改標題與內容比新開一頁好：保留既有排名歷史，也避免兩頁互相競食。
- **樞紐頁的自身價值只放在兩題**：租哪一家、多少錢。其餘全部濃縮成一段再連出去，不重述子頁內容。
- **關鍵發現（各公司官網，查核 2026-09-07）**：三家的差別在機場接駁不在車款。Times 那霸機場店接駁約 5 分鐘，OTS 與 ORIX 約 20 分鐘（來回差近 30 分鐘）。但 **Times 營業到 18:00，官網明寫 18:00 後抵達的班機無法對應**——台北出發傍晚落地的班次很多，這是地雷。OTS 是 08:00–19:00，三家最寬。
- 價格用 OTS 官網 24 小時定價當錨點（輕自動車 8,250 円起、七人座 18,700 円起），文中明寫**這是定價不是成交價**，避免讀者拿去當預算。
- 座椅資料沿用 `research/child-seat-comparison.md`（查核 2026-09-01）。
- **內鏈**：8 篇子文原本有 6 篇連回樞紐頁，補上 luggage-stroller 與 driving-pace 兩篇。
- **比較用清單不用表格**：`.table-scroll` 對表格套 640px 最小寬度，375px 螢幕只露得出兩欄、營業時間會被藏在橫捲後面。已在 375px 實測，頁面無水平溢位。
- 封面：Unsplash，作者 Zion C，**位置中繼資料為沖繩縣豐見城市瀨長島**（與 OTS／ORIX 門市同一區），畫面車輛掛日本「わ」租賃號牌。已登錄 `content/image-credits.md`。
- 閘門：`content:audit:strict` 54 篇／0 重大／25 提醒（與基準相同）；`next build` 通過、86 靜態頁。

### 下一步（依序）

1. **觀察這頁**：兩個月大的站要排 1萬–10萬的主詞是幾個月的事，不是幾週。先看 GSC 有沒有開始出現「沖繩租車」這個 query。
2. 若租車樞紐頁見效，同一套做法套到下一個主詞。**住宿與景點的 SERP 前排是大平台，租車前 5 名有 4 個個人站**，所以租車是先做的那一個。
3. `BreadcrumbList` 中文版缺（只有 `app/en/blog/[slug]/page.tsx` 有）。**`FAQPage` 不要補**——Google 2023/08 已把 FAQ 複合式結果限縮到政府與醫療類網站，旅遊部落格加了不會顯示。BreadcrumbList 仍支援，值得補，但屬順手事。
4. 未寫的內容缺口：上網與付錢（網卡／eSIM／換匯 各 0 篇）、母乳與擠奶（0 篇）。

## 2026-09-07 誤發：三篇打包清單的 15 條蝦皮連結（Claude Code，需使用者確認）

**這一段是為了讓紀錄與事實相符，不是為了補一個好看的說法。**

- 提交 `ce350d1`（2026-09-07 21:52:02 +0800，訊息為 `Correct what OTS can and cannot do after hours`）除了租車樞紐頁之外，還一併提交並推送了下列三個檔案，**但那個訊息完全沒有提到它們**：
  - `content/blog/okinawa-family-beach-packing-list.md`（5 條）
  - `content/blog/okinawa-family-medicine-sunscreen-repellent.md`（5 條）
  - `content/blog/okinawa-summer-family-packing-list.md`（5 條）
- 內容是把商品名稱改寫成指向 `s.shopee.tw` 短網址的連結，共 **15 條**。
- **原因**：Claude Code 用 `git add -A content/` 提交租車頁的修正，把工作區裡既有但未提交的改動一起掃了進去。這是操作疏失，不是刻意發布。

### 已確認的事實

- 這三個檔案上一次被提交分別是 `afa284b`（2026-08-04 13:45）與 `7ec5ad9`（2026-09-05 01:14）。**所以這 15 條連結是在 2026-09-05 01:14 之後、2026-09-07 21:52 之前寫進工作區的。**
- 更精確的時間查不出來。檔案 mtime 在 `git add` 時因行尾轉換被覆寫，已不可用；本日誌與 `000_Agent/memory/daily/2026-09-07.md` 都沒有相關紀錄。**這一項就是不知道，不要事後補一個猜測進來。**
- 不是 Claude Code 加的。本次工作只動租車樞紐頁與其連入頁。
- 線上狀態技術面正確：揭露列「本文含合作連結」有出現（`hasAffiliateLink` 認得 `shopee.tw`），5 條連結均帶 `rel="sponsored noreferrer noopener"`。

### 待使用者裁決

本檔第 759 行的既有規則是：**蝦皮連結只能在使用者核可該商品、或明確要求從後台挑選之後才加。** 本檔查不到這 15 條的核可紀錄。

- 若這 15 條**已經過核可**：補一段正式紀錄說明選品理由即可，連結留著。
- 若**尚未核可**：以一般的 revert 提交移除，不改寫歷史。
...
- **來源更正（2026-09-07 使用者告知）**：這 15 條是**另一個同時開著的 Claude Code session** 加的，不是使用者手動加的，也不是 Codex。本節初版寫成「來源是使用者本人的工作區，不是代理自行加入」，**該敘述錯誤**，以本段為準。
- **因此核可狀態仍然未知。** 那個 session 是否取得使用者核可，本 session 無從得知，不要替它假設。要確認請直接問那個分頁，或由使用者說明。
- **已於同日移除（使用者裁決）。** 使用者指出這 15 條尚未確認完整，經查證後同意：
  - **結構上完整**：三張商品表各 5 列、各 5 條連結，格式與站上既有商品表一致，不是打到一半。
  - **流程上不完整**：本檔第 619–621 行記錄過去每次加蝦皮連結都會寫下選品理由與「連結由使用者登入後台產生」。**這 15 條沒有任何紀錄**，而第 759 行的規則要求的正是核可與選品理由。
  - **判斷標準的更正**：本節初版以「線上渲染正常、揭露列有出現、`rel="sponsored"` 有掛」當作可以保留的理由。**那是錯的標準**——渲染正確與「商品選得對、使用者同意」是兩回事。
  - 處置：以 `git checkout ce350d1^ -- <三個檔案>` 還原，不改寫歷史，內容仍在 git 歷史中可取回。
...
## 2026-09-09 交接：本 session 全部工作與下一步（Claude Code）

**Active owner：無。本 session 已收工，工作區乾淨、與 origin 同步。** 下一位可直接接手。

### 一、已上線的內容改動（全部已推送並線上驗證）

**1. 租車主詞樞紐頁** `okinawa-rental-car-family-guide`（`c7a3752`、`f20435b`、`ce350d1`）

升級既有的薄總覽頁，不另開網址，避免與子頁競食。這是全站**第一個承接主詞的頁面**——關鍵字實測顯示「沖繩租車」1萬–10萬，而站上 54 篇全在長尾。

自身價值放在兩題：租哪一家、多少錢。三家差異在**機場接駁不在車款**：Times 接駁約 5 分鐘但 **18:00 後抵達的班機不受理**（官網明文）；OTS 08:00–19:00、**還車只到 18:30**、且**營業時間外可出車，19 時後抵達仍可當天取車**；ORIX 09:00–19:00 有外語櫃台但座椅四家最貴。**OTS 不在比價平台上**，只能從它的繁體中文官網訂，文中已寫明。

**2. 伴手禮打包篇改標題** `okinawa-souvenir-packing-guide`（`8bb5d7a`）

全站曝光最高（1,425）、CTR 最差（2.4%）、且是前十名唯一沒被標題修正掃到的。查詢層級證實原因：GSC 願意具名的查詢**全部是「XX 可以帶回台灣嗎」、全部 0 點擊**，而該篇內文一次都沒提過蘋果派、海帶、珊瑚。標題的「怎麼帶回台灣」是吸走海關查詢的原因，已改為「怎麼打包」，並在第二段加分流句指向海關篇。

**3. 海關篇補完並更正兩個錯誤** `okinawa-souvenir-customs-taiwan`（`57b1599`）

補七個有曝光但無內容承接的品項（黑糖、沙翁、辣油、飯糰、豆腐、年輪蛋糕、石垣牛餅乾，約 35 曝光）。

**更正兩個錯誤，這比補品項重要**：
- 原文寫「罐頭肉品一律禁止、真空密封都不例外」。**關務署（2026-05-27 更新）規定是有條件放行**：符合高溫滅菌罐製要件即可，四條件須同時成立——容器完全密閉、經高溫處理可常溫保存、**不含防腐劑**、**含適當量湯汁**。乾式午餐肉卡第四項、含硝酸鹽者卡第三項，所以實務結論不變但理由錯了。已新增專節。
- 原文把日本豬肉歸在非洲豬瘟。**日本是傳統豬瘟**，2018 年 11 月起暫停；2023 年 1 月開放的加熱豬肉產品是**商業輸入**，不適用旅客攜帶。

另補上完全沒寫過的數量上限：農畜水產品總量 **6 公斤**，食米／熟花生／熟蒜頭／乾金針／乾香菇／茶葉各 **1 公斤**。

石垣牛餅乾**刻意不給結論**：能不能帶取決於成分表有無實際牛肉，關務署把「肉餡糕餅」列為管制品項。這種題目給錯答案的代價是讀者被罰，指示是翻成分表或走紅線。

**4. 英文 car-seat 改標題** `content/blog-en/okinawa-child-car-seat-rental-guide.md`（`5b408d2`）

排序 5.7（英文區最好）但 CTR 1.3%（最差）；Where to Stay 排 8.2 卻有 4.7%。排名不是問題，標題是。中文版同一篇 9/1 已改成問句＋數字答案，英文版一直沒跟上。已比照。

**5. 中文版補 `BreadcrumbList`** `app/blog/[slug]/page.tsx`（`8f61017`）

英文版一直有、中文版沒有，54 篇全缺。已比照英文版加三層。**`FAQPage` 刻意不補**——Google 2023 年 8 月起將 FAQ 複合式結果限縮至政府與醫療類網站，旅遊部落格加了不顯示。

### 二、已決策、不要重啟的兩件事

**Klook 租車連結：不掛。** 那霸機場落地頁供應商只有 `Car rental okinawa` 與 `Hello rent a car`，**沒有 OTS／Times／ORIX／Nippon**；價格最低 NT$3,314／天、平均 NT$5,368，同期 Trip.com 標「最低 TWD619 起」。掛了等於推薦三家大公司後把讀者送到兩家沒聽過的小業者。詳見 `c6503b7`。

**英文區：不擴。** 完整量測在 `research/english-keyword-measurement-2026-09-09.md`。結論是一把剪刀：**有量的英文詞 SERP 對內容站關閉，SERP 開放的親子詞查不到量。**

- 24 個親子講法實測，**18 個無資料、3 個 10–100**，兩個對照組正常回報
- `okinawa churaumi aquarium` 1萬–10萬、`okinawa car rental` 1000–1萬，但兩者 SERP 前排全是官方站、供應商與 OTA，零內容站
- 中文「沖繩租車」是唯一「量大且 SERP 可打」的地方（前五名四個個人部落格）

**處置**：三篇英文留著不刪、car-seat 標題保留、**不寫新文章、不重新定位、churaumi 不要改標題**（導航型查詢，改了也上不去）。翻案條件寫在研究檔第五之二與第六節。

### 三、9/22 回看：五個判斷點與基準線

**這是下一位最該做的事。在此之前不要再改這些頁面**——今天四項改動需要兩週才讀得出來，再疊改動就分不清是誰造成的。

站台基準（近 28 天，資料截至約 9/2–9/6）：**452 點擊 / 1.24 萬曝光 / CTR 3.6% / 平均排序 6.9**；索引 74，未索引 9。

| 檢查 | 現在的基準 | 成功的樣子 |
| :-- | :-- | :-- |
| 伴手禮打包篇 | 1,425 曝光 / 34 點擊 / **2.4%** | CTR 往 4% 走。**曝光可能先掉，那是預期的**，代表不再攔截接不住的查詢 |
| 兩篇伴手禮合計點擊 | **43**（打包 34 ＋ 海關 9） | 高於 43。低於就是改錯了，**一個 commit 改回去，網址從未變動** |
| 租車樞紐頁 | 「沖繩租車」查詢**不存在** | GSC 出現這個 query，不論排名多後面 |
| 英文 car-seat | 154 曝光 / 2 點擊 / **1.3%** / 排 5.7 | CTR 往 4% 走 |
| 蝦皮 Sub_id | **未驗證** | Click Report 的 Sub_id 欄位不再空白（需有真實讀者點擊，**自己點會污染報告**） |

**分岔**：租車樞紐頁若吃到主詞，同一套做法套到**首頁**——它的 title 已是「2026 沖繩親子自由行攻略」，只差把「親子」這個把量縮小兩個量級的詞處理掉，而且首頁天生有全站內鏈。若沒吃到，先查是排名還是內容問題，**不要急著開第二個樞紐頁**。

### 四、順手修的兩件

- GSC 的「重新導向錯誤」1 頁是 `/en/`（帶斜線），首次偵測 8/8。實測 `/en/` → 308 → `/en` → 200，單跳乾淨，sitemap 與首頁連結都用無斜線版本，**程式面無事可修**。已按「驗證修正後的項目」，狀態顯示驗證已開始。8/23 那封 404 通知信是舊的，**目前 404 已歸零**。
- 蝦皮候選表第三節仍寫著無法執行的連字號 slug（第六節才是可用的駝峰式）。已加註指向，並把第一節三處 `article_xxx` 改為實際值。詳見 `b84eaf4`。

### 五、未做，且不要當成已完成

- **蝦皮 Sub_id 是否真的帶入，尚未驗證。** 這是那 16 條重產的主要理由。
