...
- 2026-08-04 PageSpeed mobile optimization (Codex, local work complete; deployment verification next): baseline for `https://okinawafamilynotes.com/` is Performance 60, Accessibility 95, Best Practices 81 and SEO 100; LCP is 19.3 seconds and image delivery can save about 4,288 KiB. Replaced the two homepage-linked article covers with right-sized WebP files: emergency-kit cover 2,061,467 -> 106,842 bytes and Naha-airport luggage cover 2,089,111 -> 77,994 bytes. Both article references now use WebP. Strict content audit passed with 47 articles and 0 critical issues; production `next build` passed with 72 static pages. Local 375px browser QA passed for both articles: images loaded at their intended dimensions and document client/scroll widths were both 375px. Codex owns only `100_Todo/plans/2026-08-04-pagespeed-mobile-baseline.md`, these two cover assets and references, plus this handoff. After deployment, repeat the same production mobile PageSpeed test and record the comparison. Preserve all unrelated dirty files.
