...
- 2026-08-06 cover-caption simplification (Codex, pending verification): reader-facing captions must identify the scene only; removed source/provenance and "real photo" wording from the DMM stingray cover and Okinawa World entrance cover. New captions: `館內水槽的魟魚。` and `沖繩世界園區入口。`. Before publishing, run the strict audit and production build, then verify both at a 390px production viewport.
