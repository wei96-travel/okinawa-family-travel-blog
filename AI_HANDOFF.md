...
- 2026-08-14 affiliate experiment (Codex, active): Codex owns `100_Todo/plans/2026-08-14-affiliate-60-day-baseline.md`, `app/layout.tsx` and this handoff entry. Current scope is measurement and link verification only; do not add Agoda, move affiliate placements or rewrite public articles until the baseline and GA4 reception check are complete. Local production build and strict content audit passed. Browser testing confirmed the Churaumi Klook and Trip.com tracked links open separate tabs, preserve the article tab, retain `rel="sponsored noreferrer noopener"`, and preserve the expected platform tracking parameters. A fresh production-page inspection found that `window.gtag` and `window.dataLayer` were both absent: Vercel was not supplying `NEXT_PUBLIC_GA_ID`, so `affiliate_click` could not be sent at all. `app/layout.tsx` now keeps the environment variable as first choice and falls back to the project's existing public GA4 measurement ID `G-Z076ZYK89C`; the 79-page production build passed. Next action: push this narrow fix, verify the live page loads GA, click one tracked link and confirm `affiliate_click` in GA4 Realtime/DebugView.
