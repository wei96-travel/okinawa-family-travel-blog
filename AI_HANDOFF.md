...
- 2026-08-13 那霸雨天決策文（Codex，已提交待推送）：使用者已核可完整文章版本。提交 `fc857fe`（`Publish Naha rainy-day decision guide`）僅加入 `content/blog/okinawa-naha-rainy-indoor-decision-guide.md`、內容計畫、草稿與三輪檢查紀錄；文章以 DMM Kariyushi 水族館與沖繩縣立博物館・美術館的二選一為核心，未新增圖片、票券、Klook 或其他分潤連結。嚴格稽核通過（49 篇、0 個重大問題；37 個既有提醒），production build 通過並產生 77 個靜態頁；等待推送後的正式站、canonical、sitemap 與索引驗證。Facebook〈下雨不是把景點全部搬室內〉與手機預覽仍未上傳、未排程、未發布，因為使用者尚未明確要求社群發布。
