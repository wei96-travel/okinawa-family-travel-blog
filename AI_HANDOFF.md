...
- 2026-08-01 Okinawa family budget calculator MVP (Codex, local prototype completed; awaiting user review): added a simple mobile-first calculator for adults, children, trip length, travel style, transport choice and optional total airfare. It returns an `NT$` planning range, per-person estimate and itemized breakdown, supports URL-based sharing without a database or personal data, links to the existing booking hub, and records GA4 interaction and premium-interest events. It does not add accounts, payment, live booking prices or public deployment. `content:audit:strict` passed with 46 articles and 0 critical issues; production build passed with 71 static pages. Browser QA passed at 375, 768, 1024 and 1440 widths with no horizontal overflow, correct query-string restoration, one main landmark, 44px counter controls, visible `NT$` labels and no console errors. Claimed files: `app/okinawa-family-trip-budget-calculator/page.tsx`, `components/okinawa-budget-calculator.tsx`, `app/sitemap.ts`, `components/footer.tsx`, `100_Todo/plans/2026-08-01-okinawa-budget-calculator-mvp.md`, and this handoff. Do not deploy until the user approves the local prototype.
