...
Last updated: 2026-08-01 (Codex, AdSense account created and site connection deploying)
...
- 2026-08-01 AdSense account and site connection (Codex, account created; deployment commit `f65833a`): created the AdSense account under `wei960315@gmail.com` for `okinawafamilynotes.com`; publisher ID is `pub-8772944565154374`. Added the exact AdSense verification script supplied by the account to the root layout and published the exact account-provided line in `public/ads.txt`. Production `next build` passed with 70 static pages; a fresh local server returned the publisher ID in the homepage HTML and the exact `ads.txt` line at `/ads.txt`. Do not create ad placements or enable broad automatic ads before site review is approved. Remaining account steps are production verification, review submission and payment-profile details; payment information must be entered with the user because it contains legal and banking data.
