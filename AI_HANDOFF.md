...
- 2026-08-06 DMM original-photo download deterrence (Codex, ready to deploy): user requested download protection after publication. Added `ProtectedOriginalImage`, applied only to the DMM article's original-photo cover and inline images, and added CSS to block ordinary right-click saving, drag saving, text selection and iOS-style touch callouts. This is deliberately a deterrent, not DRM: a browser must still receive an image in order to display it. `content:audit:strict` passed (48 articles, 0 critical; 37 existing style reminders) and production `next build` passed (73 static pages). Commit only `app/blog/[slug]/page.tsx`, `app/globals.css`, `components/markdown-content.tsx`, `components/protected-original-image.tsx` and this handoff entry; preserve all other dirty files.
