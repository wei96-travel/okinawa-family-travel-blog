...
- 2026-08-06 Gyokusendo name correction (Codex, pending verification): a reader-facing caption mistakenly said `岩宿王`; the correct name is `岩窟王`, verified against the sign in wei96's supplied photo and independent travel references. Corrected the public alt/caption, the content plan and this handoff; before publishing, run the strict audit and production build, then verify the exact caption at a 390px production viewport. Do not treat visual recognition of Japanese text as sufficient: cross-check every named photo landmark against its sign or authoritative reference before publishing.
