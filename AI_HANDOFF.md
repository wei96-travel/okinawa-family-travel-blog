...
Last updated: 2026-08-14 (Codex, affiliate measurement restored and verified on production)

- 2026-08-30 SEO repair release (Codex, local verification complete): user approved a narrowly scoped deployment to resolve the live `/affiliate-disclosure` 404, expose `/newsletter` through an honest footer label, retain both routes in the sitemap, and add a relevant Open Graph/Twitter image to the family-trip budget calculator. Claimed files: `app/affiliate-disclosure/page.tsx`, `components/footer.tsx`, `app/sitemap.ts`, `app/okinawa-family-trip-budget-calculator/page.tsx`, and this handoff. No article, social asset, analytics implementation or unrelated dirty file is in scope. A bounded public-summary Claude Code review agreed these can ship together but must be verified separately. Mobile checks passed at 390px with no horizontal overflow; the disclosure route, footer links, canonical, sitemap entries and calculator social metadata were verified locally. Strict content audit passed with 49 articles and 0 critical issues, `git diff --check` passed, and the production build generated all 80 static pages. Commit, push and production verification are the remaining release steps.
