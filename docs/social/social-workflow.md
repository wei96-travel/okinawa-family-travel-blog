...
2. **Facebook 獨立長文**（1,000～1,500 字，輪播完成後必寫）：用同一篇網站文章另寫一篇新貼文，結構是具體處境 → 痛點展開 → 兩到四個可執行做法 → 一段收束 → 「連結放留言」→ hashtag。長文要補足圖片放不下的判斷、限制與使用情境，不能只是把輪播八張逐字貼回來。**貼文本體不放連結**，CTA 與連結寫成第一則留言，發文後立刻補。
