...
6. 跑一次「生圖後檢查表」。
7. **8 張輪播至少採用 3 個不同的 GPT 寫實情境場景；封面與 CTA 必須是滿版情境照片。** 資訊圖只能協助照片無法誠實表達的規則、費率、時間或比較資料，不能替代主要視覺。
...
2. **Facebook 獨立長文**（1,000～1,500 字）：另外選一個當週尚未做成輪播的新切角，結構是具體處境 → 痛點展開 → 兩到四個可執行做法 → 一段收束 → 「連結放留言」→ hashtag。可以導回同一篇或另一篇相關網站文章，但不能只是把輪播八張改寫成長文，也不要在輪播發出隔天立刻重講同一個主題。**貼文本體不放連結**，CTA 與連結寫成第一則留言，發文後立刻補。
...
**規則**：四個平台的第一句不要完全一樣，避免同一批人看到重複內容。Facebook 長文還要和最近發布的輪播核對題目與 Hook；如果核心問題相同，就換新切角或延後發布。
