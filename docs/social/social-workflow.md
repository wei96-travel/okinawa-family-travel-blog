...
- Facebook 的貼文、輪播、Reels、第一則留言與洞察，一律使用 **Codex 內建瀏覽器**。
- 不要讓系統自動選擇 Chrome，也不要因為 Facebook 網址已在 Chrome 開啟就切換流程。
- 只有使用者在當次任務明確指定 Chrome 時，才可改用 Chrome。
- 內建瀏覽器連線或上傳失敗時，立即停在發佈前回報；不得自行切到 Chrome、不得用其他瀏覽器繞過，也不得回報「已發布」。
- 發佈輪播前，先確認預覽中實際出現全部圖片；8 張輪播必須核對 `01.png` 到 `08.png` 都已上傳。如果預覽少圖、只有文字、順序錯誤或載入異常，立即返回，不得發佈。
- 最後按下「發佈」前，再確認分享對象為「所有人」、圖片順序正確，且主貼文沒有外部連結。
- 發佈後重新確認公開貼文可見，並確認作者第一則留言已出現正確的網站網址；兩項都完成才算發布成功。
